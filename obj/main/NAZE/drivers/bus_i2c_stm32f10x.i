...
# 1 "./src/main/drivers/bus_i2c_stm32f10x.c"
# 19 "./src/main/drivers/bus_i2c_stm32f10x.c"
...
# 42 "./src/main/drivers/bus_i2c_stm32f10x.c"
...
static const i2cDevice_t i2cHardwareMap[] = {
    { ((I2C_TypeDef *) (((uint32_t)0x40000000) + 0x5400)), ((GPIO_TypeDef *) ((((uint32_t)0x40000000) + 0x10000) + 0x0C00)), Pin_6, Pin_7, I2C1_EV_IRQn, I2C1_ER_IRQn, ((uint32_t)0x00200000) },
    { ((I2C_TypeDef *) (((uint32_t)0x40000000) + 0x5800)), ((GPIO_TypeDef *) ((((uint32_t)0x40000000) + 0x10000) + 0x0C00)), Pin_10, Pin_11, I2C2_EV_IRQn, I2C2_ER_IRQn, ((uint32_t)0x00400000) },
};


static I2C_TypeDef *I2Cx = ((void *)0);

static I2CDevice I2Cx_index;
static _Bool i2cOverClock;

void i2cSetOverclock(uint8_t OverClock)
{
    i2cOverClock = (OverClock) ? 1 : 0;
}
...
static volatile _Bool error = 0;
static volatile _Bool busError = 0;
static volatile _Bool busy;
...
static _Bool i2cHandleHardwareFailure(void)
{
    i2cErrorCount++;
...
_Bool i2cWriteBuffer(uint8_t addr_, uint8_t reg_, uint8_t len_, uint8_t *data)
{
    uint32_t timeout = ((uint32_t)(10 * ((uint32_t)0x1000)));
...
_Bool i2cWrite(uint8_t addr_, uint8_t reg_, uint8_t data)
{
    return i2cWriteBuffer(addr_, reg_, 1, &data);
}

_Bool i2cRead(uint8_t addr_, uint8_t reg_, uint8_t len, uint8_t* buf)
{
    uint32_t timeout = ((uint32_t)(10 * ((uint32_t)0x1000)));
...
static void i2c_er_handler(void)
{
    ...
    if (SR1Register & 0x0F00) {
        ...
        if (SR1Register & ((uint16_t)0x0100))
            busError = 1;
    }
    ...
    if (SR1Register & 0x0700) {
        ...
        if (!(SR1Register & 0x0200) && !(I2Cx->CR1 & 0x0200)) {
            if (I2Cx->CR1 & 0x0100) {
...
void i2c_ev_handler(void)
{
    ...
    if (SReg_1 & 0x0001) {
        ...
        index = 0;
        if (reading && (subaddress_sent || 0xFF == reg)) {
            subaddress_sent = 1;
            if (bytes == 2)
                I2Cx->CR1 |= 0x0800;
            I2C_Send7bitAddress(I2Cx, addr, ((uint8_t)0x01));
        } else {
            I2C_Send7bitAddress(I2Cx, addr, ((uint8_t)0x00));
            if (reg != 0xFF)
                index = -1;
        }
...
void i2cInit(I2CDevice index)
{
    ...
    I2C_ITConfig(I2Cx, ((uint16_t)0x0200) | ((uint16_t)0x0100), DISABLE);
    i2c.I2C_Mode = ((uint16_t)0x0000);
    i2c.I2C_DutyCycle = ((uint16_t)0xBFFF);
    i2c.I2C_AcknowledgedAddress = ((uint16_t)0x4000);
    ...
    nvic.NVIC_IRQChannelSubPriority = (((((((((0)<<(4-(7-(((uint32_t)0x500)>>8))))|((0)&(0x0f>>(7-(((uint32_t)0x500)>>8)))))<<4)&0xf0))>>4)&(0x0f>>(7-(((uint32_t)0x500)>>8)))));
