...
void ws2811LedStripInit(void)
{
    ...
    dmaSetHandler((&dmaChannels[5]), &ws2811DMAHandlerRec, (((((1)<<(4-(7-(((uint32_t)0x500)>>8))))|((2)&(0x0f>>(7-(((uint32_t)0x500)>>8)))))<<4)&0xf0));
...
static void fastUpdateLEDDMABuffer(uint8_t **buffer, rgbColor24bpp_t color)
{
    uint32_t grb = (color.rgb.g << 16) | (color.rgb.r << 8) | (color.rgb.b);

    for (int bit = 0; bit < 24; bit++) {
        *(*buffer)++ = (grb & (1 << 23)) ? 17 : 9;
        grb <<= 1;
    }
}
