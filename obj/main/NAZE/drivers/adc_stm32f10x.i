...
# 25 "./src/main/drivers/adc_stm32f10x.c" 2
...
extern adc_config_t adcConfig[(ADC_CHANNEL_MAX + 1)];
extern volatile uint16_t adcValues[(ADC_CHANNEL_MAX + 1)];
...
# 54 "./src/main/drivers/adc_stm32f10x.c"
void adcInit(drv_adc_config_t *init)
{
    ...
    if (init->enableVBat) {
        ...
        adcConfig[ADC_BATTERY].adcChannel = ((uint8_t)0x04);
        adcConfig[ADC_BATTERY].dmaIndex = configuredAdcChannels++;
        adcConfig[ADC_BATTERY].enabled = 1;
        adcConfig[ADC_BATTERY].sampleTime = ((uint8_t)0x07);
    }
    ...
    if (init->enableRSSI) {
        ...
        GPIO_Init(((GPIO_TypeDef *) ((((uint32_t)0x40000000) + 0x10000) + 0x0800)), &GPIO_InitStructure);
        adcConfig[ADC_RSSI].adcChannel = ((uint8_t)0x01);
        adcConfig[ADC_RSSI].dmaIndex = configuredAdcChannels++;
        adcConfig[ADC_RSSI].enabled = 1;
        adcConfig[ADC_RSSI].sampleTime = ((uint8_t)0x07);
    }
    ...
    if (init->enableExternal1) {
        ...
        GPIO_Init(((GPIO_TypeDef *) ((((uint32_t)0x40000000) + 0x10000) + 0x0800)), &GPIO_InitStructure);
        adcConfig[ADC_EXTERNAL1].adcChannel = ((uint8_t)0x05);
        adcConfig[ADC_EXTERNAL1].dmaIndex = configuredAdcChannels++;
        adcConfig[ADC_EXTERNAL1].enabled = 1;
        adcConfig[ADC_EXTERNAL1].sampleTime = ((uint8_t)0x07);
    }
    ...
    if (init->enableCurrentMeter) {
        ...
        adcConfig[ADC_CURRENT].adcChannel = ((uint8_t)0x09);
        adcConfig[ADC_CURRENT].dmaIndex = configuredAdcChannels++;
        adcConfig[ADC_CURRENT].enabled = 1;
        adcConfig[ADC_CURRENT].sampleTime = ((uint8_t)0x07);
    }
    ...
    uint8_t rank = 1;
    for (i = 0; i < (ADC_CHANNEL_MAX + 1); i++) {
        if (!adcConfig[i].enabled) {
            continue;
        }
        ADC_RegularChannelConfig(((ADC_TypeDef *) ((((uint32_t)0x40000000) + 0x10000) + 0x2400)), adcConfig[i].adcChannel, rank++, adcConfig[i].sampleTime);
    }
