...
# 1 "./src/main/drivers/serial_softserial.c"
# 18 "./src/main/drivers/serial_softserial.c"
...
# 19 "./src/main/drivers/serial_softserial.c" 2
...
# 20 "./src/main/drivers/serial_softserial.c" 2
...
# 21 "./src/main/drivers/serial_softserial.c" 2
...
# 23 "./src/main/drivers/serial_softserial.c" 2
...
# 29 "./src/main/drivers/serial_softserial.c" 2
...
# 30 "./src/main/drivers/serial_softserial.c" 2
...
# 33 "./src/main/drivers/serial_softserial.c" 2
...
# 35 "./src/main/drivers/serial_softserial.c" 2
...
# 37 "./src/main/drivers/serial_softserial.c" 2
...
static _Bool isTimerPeriodTooLarge(uint32_t timerPeriod)
{
...
static void serialTimerTxConfig(const timerHardware_t *timerHardwarePtr, uint8_t reference, uint32_t baud)
{
    ...
    timerChConfigCallbacks(timerHardwarePtr, &softSerialPorts[reference].timerCb, ((void *)0));
}
...
static void serialTimerRxConfig(const timerHardware_t *timerHardwarePtr, uint8_t reference, portOptions_t options)
{
    ...
    serialICConfig(timerHardwarePtr->tim, timerHardwarePtr->channel, (options & SERIAL_INVERTED) ? ((uint16_t)0x0000) : ((uint16_t)0x0002));
    timerChCCHandlerInit(&softSerialPorts[reference].edgeCb, onSerialRxPinChange);
    timerChConfigCallbacks(timerHardwarePtr, &softSerialPorts[reference].edgeCb, ((void *)0));
}
...
serialPort_t *openSoftSerial(softSerialPortIndex_e portIndex, serialReceiveCallbackPtr callback, uint32_t baud, portOptions_t options)
{
    ...
    if (portIndex == SOFTSERIAL1) {
        softSerial->rxTimerHardware = &(timerHardware[4]);
        softSerial->txTimerHardware = &(timerHardware[5]);
    }



    if (portIndex == SOFTSERIAL2) {
        softSerial->rxTimerHardware = &(timerHardware[6]);
        softSerial->txTimerHardware = &(timerHardware[7]);
    }
    ...
    softSerial->isTransmittingData = 0;

    softSerial->isSearchingForStartBit = 1;
...
void processTxState(softSerial_t *softSerial)
{
    ...
    if (!softSerial->isTransmittingData) {
        ...
        softSerial->isTransmittingData = 1;
...
void prepareForNextRxByte(softSerial_t *softSerial)
{
    ...
    if (softSerial->rxEdge == LEADING) {
        ...
        serialICConfig(
            ...
            (softSerial->port.options & SERIAL_INVERTED) ? ((uint16_t)0x0000) : ((uint16_t)0x0002)
        );
    }
}
...
{
    (void)(capture);
    softSerial_t *softSerial = ( __extension__ ({ const typeof( ((softSerial_t *)0)->timerCb ) *__mptr = (cbRec); (softSerial_t *)( (char *)__mptr - __builtin_offsetof (softSerial_t, timerCb) );}));
...
void onSerialRxPinChange(timerCCHandlerRec_t *cbRec, captureCompare_t capture)
{
    ...
    softSerial_t *softSerial = ( __extension__ ({ const typeof( ((softSerial_t *)0)->edgeCb ) *__mptr = (cbRec); (softSerial_t *)( (char *)__mptr - __builtin_offsetof (softSerial_t, edgeCb) );}));
    _Bool inverted = softSerial->port.options & SERIAL_INVERTED;
    ...
    if (softSerial->isSearchingForStartBit) {
        ...
        serialICConfig(softSerial->rxTimerHardware->tim, softSerial->rxTimerHardware->channel, inverted ? ((uint16_t)0x0002) : ((uint16_t)0x0000));
        softSerial->rxEdge = LEADING;

        softSerial->rxBitIndex = 0;
        softSerial->rxLastLeadingEdgeAtBitIndex = 0;
        softSerial->internalRxBuffer = 0;
        softSerial->isSearchingForStartBit = 0;
        return;
    }
    ...
    if (softSerial->rxEdge == TRAILING) {
        ...
        softSerial->rxEdge = TRAILING;
        serialICConfig(softSerial->rxTimerHardware->tim, softSerial->rxTimerHardware->channel, inverted ? ((uint16_t)0x0000) : ((uint16_t)0x0002));
    }
}
...
_Bool isSoftSerialTransmitBufferEmpty(serialPort_t *instance)
{
