...
# 26 "./src/main/drivers/pwm_output.c" 2
...
# 29 "./src/main/drivers/pwm_output.c" 2
...
# 31 "./src/main/drivers/pwm_output.c" 2
...
# 33 "./src/main/drivers/pwm_output.c" 2
...
static pwmOutputPort_t pwmOutputPorts[12];

static pwmOutputPort_t *motors[12];
...
static void pwmOCConfig(TIM_TypeDef *tim, uint8_t channel, uint16_t value)
{
    ...
    switch (channel) {
        case ((uint16_t)0x0000):
            ...
            TIM_OC1PreloadConfig(tim, ((uint16_t)0x0008));
            break;
        case ((uint16_t)0x0004):
            TIM_OC2Init(tim, &TIM_OCInitStructure);
            TIM_OC2PreloadConfig(tim, ((uint16_t)0x0008));
            break;
        case ((uint16_t)0x0008):
            TIM_OC3Init(tim, &TIM_OCInitStructure);
            TIM_OC3PreloadConfig(tim, ((uint16_t)0x0008));
            break;
        case ((uint16_t)0x000C):
            TIM_OC4Init(tim, &TIM_OCInitStructure);
            TIM_OC4PreloadConfig(tim, ((uint16_t)0x0008));
            break;
    }
}
...
{
    uint8_t index;
    TIM_TypeDef *lastTimerPtr = ((void *)0);
...
{
    uint32_t hz = 8 * 1000000;
    motors[motorIndex] = pwmOutConfig(timerHardware, 8, hz / motorPwmRate, idlePulse);
    motors[motorIndex]->pwmWritePtr = pwmWriteBrushed;
}

void pwmBrushlessMotorConfig(const timerHardware_t *timerHardware, uint8_t motorIndex, uint16_t motorPwmRate, uint16_t idlePulse)
{
    uint32_t hz = 1 * 1000000;
    motors[motorIndex] = pwmOutConfig(timerHardware, 1, hz / motorPwmRate, idlePulse);
    motors[motorIndex]->pwmWritePtr = pwmWriteStandard;
}

void pwmOneshotMotorConfig(const timerHardware_t *timerHardware, uint8_t motorIndex)
{
    motors[motorIndex] = pwmOutConfig(timerHardware, 8, 0xFFFF, 0);
    motors[motorIndex]->pwmWritePtr = pwmWriteStandard;
}
...
{
    servos[servoIndex] = pwmOutConfig(timerHardware, 1, 1000000 / servoPwmRate, servoCenterPulse);
}

void pwmWriteServo(uint8_t index, uint16_t value)
{
    if (servos[index] && index < 8)
