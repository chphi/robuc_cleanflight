...
# 32 "./src/main/sensors/voltage.c" 2
...
# 36 "./src/main/sensors/voltage.c" 2
...
voltageMeterState_t voltageMeterStates[1];
...
extern void pgResetFn_voltageMeterConfig(voltageMeterConfig_t *); voltageMeterConfig_t voltageMeterConfig_SystemArray[1]; extern const pgRegistry_t voltageMeterConfig_Registry; const pgRegistry_t voltageMeterConfig_Registry __attribute__ ((section(".pg_registry"), used, aligned(4))) = { .pgn = 49 | (0 << 12), .size = (sizeof(voltageMeterConfig_t) * 1) | PGR_SIZE_SYSTEM_FLAG, .address = (uint8_t*)&voltageMeterConfig_SystemArray, .ptr = 0, .reset = {.fn = (pgResetFunc*)&pgResetFn_voltageMeterConfig}, };

void pgResetFn_voltageMeterConfig(voltageMeterConfig_t *instance)
{
    for (int i = 0; i < 1; i++) {
        static const voltageMeterConfig_t _reset_template_56 = { .vbatscale = 110, .vbatresdivval = 10, .vbatresdivmultiplier = 1, }; memcpy((&instance[i]), &_reset_template_56, sizeof(*(&instance[i])));
...
static uint16_t voltageAdcToVoltage(const uint16_t src, voltageMeterConfig_t *config)
{
...
void voltageMeterInit(void)
{
    for (uint8_t i = 0; i < 1 && i < (sizeof(voltageMeterAdcChannelMap) / sizeof((voltageMeterAdcChannelMap)[0])); i++) {
        ...
        biquadFilterInitLPF(&state->vbatFilterState, 1.0f, 50000);
    }
}
