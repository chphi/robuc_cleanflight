...
static int16_t gyroADCRaw[3];
static int32_t gyroZero[3] = { 0, 0, 0 };

static uint16_t calibratingG = 0;

static biquadFilter_t gyroFilterLPF[3];
static biquadFilter_t gyroFilterNotch[3];
static pt1Filter_t gyroFilterPt1[3];
...
extern const gyroConfig_t pgResetTemplate_gyroConfig; gyroConfig_t gyroConfig_System; extern const pgRegistry_t gyroConfig_Registry; const pgRegistry_t gyroConfig_Registry __attribute__ ((section(".pg_registry"), used, aligned(4))) = { .pgn = 10 | (0 << 12), .size = sizeof(gyroConfig_t) | PGR_SIZE_SYSTEM_FLAG, .address = (uint8_t*)&gyroConfig_System, .ptr = 0, .reset = {.ptr = (void*)&pgResetTemplate_gyroConfig}, };

const gyroConfig_t pgResetTemplate_gyroConfig __attribute__ ((section(".pg_resetdata"), used, aligned(2))) = { .gyro_lpf = 0, .gyro_soft_type = FILTER_PT1, .gyro_soft_lpf_hz = 90, .gyro_soft_notch_hz = 0, .gyro_soft_notch_cutoff_hz = 130, .gyro_sync = 1, .pid_process_denom = 1, .gyro_sample_hz = 1000, .gyroMovementCalibrationThreshold = 32, }
# 96 "./src/main/sensors/gyro.c"
 ;

void gyroInit(void)
{
    if (gyroConfig()->gyro_soft_lpf_hz) {
        const uint16_t gyroPeriodUs = (1000000 / (gyro.sampleFrequencyHz));
...
static _Bool isOnFinalGyroCalibrationCycle(void)
{
    return calibratingG == 1;
}

static _Bool isOnFirstGyroCalibrationCycle(void)
{
    return calibratingG == 1000;
}
...
static void performAcclerationCalibration(uint8_t gyroMovementCalibrationThreshold)
{
    ...
    for (int axis = 0; axis < 3; axis++) {
        ...
        if (isOnFinalGyroCalibrationCycle()) {
            ...
            gyroZero[axis] = (g[axis] + (1000 / 2)) / 1000;
        }
    }
