...
# 61 "./src/main/telemetry/hott.c" 2
...
# 62 "./src/main/telemetry/hott.c" 2
...
# 73 "./src/main/telemetry/hott.c" 2
...
# 88 "./src/main/telemetry/hott.c" 2
...
# 89 "./src/main/telemetry/hott.c" 2



extern const hottTelemetryConfig_t pgResetTemplate_hottTelemetryConfig; hottTelemetryConfig_t hottTelemetryConfig_System; extern const pgRegistry_t hottTelemetryConfig_Registry; const pgRegistry_t hottTelemetryConfig_Registry __attribute__ ((section(".pg_registry"), used, aligned(4))) = { .pgn = 33 | (0 << 12), .size = sizeof(hottTelemetryConfig_t) | PGR_SIZE_SYSTEM_FLAG, .address = (uint8_t*)&hottTelemetryConfig_System, .ptr = 0, .reset = {.ptr = (void*)&pgResetTemplate_hottTelemetryConfig}, };

const hottTelemetryConfig_t pgResetTemplate_hottTelemetryConfig __attribute__ ((section(".pg_resetdata"), used, aligned(2))) = { .hottAlarmSoundInterval = 5, }
...
static _Bool hottIsSending = 0;

static uint8_t *hottMsg = ((void *)0);
...
static serialPort_t *hottPort = ((void *)0);
static serialPortConfig_t *portConfig;

static _Bool hottTelemetryEnabled = 0;
...
void hottPrepareGPSResponse(HOTT_GPS_MSG_t *hottGPSMessage)
{
    ...
    addGPSCoordinates(hottGPSMessage, GPS_coord[0], GPS_coord[1]);
    ...
    uint16_t hottGpsAltitude = GPS_altitude + 500;
...
static _Bool shouldTriggerBatteryAlarmNow(void)
{
    return ((millis() - lastHottAlarmSoundTime) >= (hottTelemetryConfig()->hottAlarmSoundInterval * 1000));
}
...
void configureHoTTTelemetryPort(void)
{
    ...
    hottPort = openSerialPort(portConfig->identifier, FUNCTION_TELEMETRY_HOTT, ((void *)0), 19200, MODE_RX, SERIAL_NOT_INVERTED);

    if (!hottPort) {
        return;
    }

    hottTelemetryEnabled = 1;
}
...
{
    static _Bool lookingForRequest = 1;
    ...
    if (lookingForRequest) {
        ...
        _Bool enoughTimePassed = currentMicros - lastHoTTRequestCheckAt >= 4000;
...
static void workAroundForHottTelemetryOnUsart(serialPort_t *instance, portMode_t mode) {
 closeSerialPort(hottPort);
 hottPort = openSerialPort(instance->identifier, FUNCTION_TELEMETRY_HOTT, ((void *)0), 19200, mode, SERIAL_NOT_INVERTED);
}
...
static inline _Bool shouldPrepareHoTTMessages(uint32_t currentMicros)
{
    return currentMicros - lastMessagesPreparedAt >= ((1000 * 1000) / 5);
}

static inline _Bool shouldCheckForHoTTRequest()
{
    if (hottIsSending) {
        return 0;
    }
...
_Bool checkHoTTTelemetryState(void)
{
    _Bool newTelemetryEnabledValue = telemetryDetermineEnabledState(hottPortSharing);
