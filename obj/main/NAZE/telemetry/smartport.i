...
# 14 "./src/main/telemetry/smartport.c" 2
...
# 40 "./src/main/telemetry/smartport.c" 2
...
# 336 "./src/main/telemetry/smartport.c"
                    static uint8_t currentCell = 0;


                    uint32_t payload = 0;
                    payload |= ((uint16_t)(vbat * 100 + batteryCellCount) / (batteryCellCount * 2)) & 0x0FFF;
                    payload <<= 4;
                    payload |= (uint8_t)batteryCellCount & 0x0F;
                    payload <<= 4;
                    payload |= (uint8_t)currentCell & 0x0F;
            ...
            case FSSP_DATAID_CURRENT :
                if (feature(FEATURE_AMPERAGE_METER)) {
                    ...
                    smartPortSendPackage(id, state->amperage / 10);
                    smartPortHasRequest = 0;
                }
                break;
            ...
            case FSSP_DATAID_FUEL :
                if (feature(FEATURE_AMPERAGE_METER)) {
                    amperageMeter_t *state = getAmperageMeter(batteryConfig()->amperageMeterSource);
                    smartPortSendPackage(id, state->mAhDrawn);
                    smartPortHasRequest = 0;
                }
                break;
            ...
            case FSSP_DATAID_ACCX :
                smartPortSendPackage(id, 100 * accSmooth[X] / acc.acc_1G);
                smartPortHasRequest = 0;
                break;
            ...
            case FSSP_DATAID_T1 :
                ...
                if ((flightModeFlags & (HORIZON_MODE)))
                    tmpi += 20;
                if ((flightModeFlags & (GTUNE_MODE)) || (flightModeFlags & (PASSTHRU_MODE)))
                    tmpi += 40;
