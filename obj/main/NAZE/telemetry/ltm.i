...
static void ltm_gframe(void)
{
    ...
    ltm_serialise_32(GPS_coord[0]);
    ltm_serialise_32(GPS_coord[1]);
...
static void ltm_oframe()
{
    ...
    ltm_serialise_32(GPS_home[0]);
    ltm_serialise_32(GPS_home[1]);
...
void configureLtmTelemetryPort(void)
{
    ...
    ltmPort = openSerialPort(portConfig->identifier, FUNCTION_TELEMETRY_LTM, ((void *)0), baudRates[baudRateIndex], MODE_TX, SERIAL_NOT_INVERTED);
    if (!ltmPort)
        return;
    ltmEnabled = 1;
}
...
void checkLtmTelemetryState(void)
{
    _Bool newTelemetryEnabledValue = telemetryDetermineEnabledState(ltmPortSharing);
