...
static uint32_t realtimeGuardInterval = 300;
...
void taskSystem(void)
{
    ...
    for (taskId = 0; taskId < TASK_COUNT; taskId++) {
        if (cfTasks[taskId].staticPriority != TASK_PRIORITY_REALTIME) {
            maxNonRealtimeTaskTime = ((maxNonRealtimeTaskTime) > (cfTasks[taskId].averageExecutionTime) ? (maxNonRealtimeTaskTime) : (cfTasks[taskId].averageExecutionTime));
        }
    }

    realtimeGuardInterval = constrain(maxNonRealtimeTaskTime, 10, 300) + 25;
...
void rescheduleTask(cfTaskId_e taskId, uint32_t newPeriodMicros)
{
    ...
    if (taskId < TASK_COUNT) {
        cfTasks[taskId].desiredPeriod = ((100) > (newPeriodMicros) ? (100) : (newPeriodMicros));
    }
}
...
void scheduler(void)
{
    ...
    uint32_t timeToNextRealtimeTask = 4294967295UL;
    ...
    for (taskId = 0; taskId < TASK_COUNT; taskId++) {
        if (cfTasks[taskId].staticPriority == TASK_PRIORITY_REALTIME) {
            ...
            if ((int32_t)(currentTime - nextExecuteAt) >= 0) {
                ...
                timeToNextRealtimeTask = ((timeToNextRealtimeTask) < (newTimeInterval) ? (timeToNextRealtimeTask) : (newTimeInterval));
            }
        }
    }

    _Bool outsideRealtimeGuardInterval = (timeToNextRealtimeTask > realtimeGuardInterval);


    for (taskId = 0; taskId < TASK_COUNT; taskId++) {
        if (cfTasks[taskId].isEnabled) {

            if (cfTasks[taskId].checkFunc != ((void *)0)) {
            ...
            cfTasks[taskId].dynamicPriority = ((cfTasks[taskId].dynamicPriority) < (TASK_PRIORITY_MAX) ? (cfTasks[taskId].dynamicPriority) : (TASK_PRIORITY_MAX));;

            _Bool taskCanBeChosenForScheduling =
    ...
    if (selectedTaskId != TASK_NONE) {
        ...
        if (cfTasks[selectedTaskId].taskFunc != ((void *)0)) {
        ...
        cfTasks[selectedTaskId].totalExecutionTime += taskExecutionTime;
        cfTasks[selectedTaskId].maxExecutionTime = ((cfTasks[selectedTaskId].maxExecutionTime) > (taskExecutionTime) ? (cfTasks[selectedTaskId].maxExecutionTime) : (taskExecutionTime));
