...
# 59 "./src/main/mw.c" 2
# 1 "./src/main/io/rc_curves.h" 1
# 18 "./src/main/io/rc_curves.h"
...
void updateGtuneState(void)
{
    ...
    if (((1 << (BOXGTUNE)) & rcModeActivationMask)) {
...
void annexCode(void)
{
    ...
    for (axis = 0; axis < 3; axis++) {
        tmp = ((((rcData[axis] - masterConfig.rxConfig.midrc) > 0 ? (rcData[axis] - masterConfig.rxConfig.midrc) : -(rcData[axis] - masterConfig.rxConfig.midrc))) < (500) ? (((rcData[axis] - masterConfig.rxConfig.midrc) > 0 ? (rcData[axis] - masterConfig.rxConfig.midrc) : -(rcData[axis] - masterConfig.rxConfig.midrc))) : (500));
    ...
    tmp = constrain(rcData[THROTTLE], masterConfig.rxConfig.mincheck, 2000);
    tmp = (uint32_t)(tmp - masterConfig.rxConfig.mincheck) * 1000 / (2000 - masterConfig.rxConfig.mincheck);
    tmp2 = tmp / 100;
    ...
    if ((armingFlags & (ARMED))) {
        digitalLo(((GPIO_TypeDef *) ((((uint32_t)0x40000000) + 0x10000) + 0x0C00)), Pin_3);
    } else {
        if (((1 << (BOXARM)) & rcModeActivationMask) == 0) {
...
void releaseSharedTelemetryPorts(void) {
    serialPort_t *sharedPort = findSharedSerialPort((FUNCTION_TELEMETRY_FRSKY | FUNCTION_TELEMETRY_HOTT | FUNCTION_TELEMETRY_SMARTPORT | FUNCTION_TELEMETRY_LTM), FUNCTION_MSP);
    while (sharedPort) {
        mspReleasePortIfAllocated(sharedPort);
        sharedPort = findNextSharedSerialPort((FUNCTION_TELEMETRY_FRSKY | FUNCTION_TELEMETRY_HOTT | FUNCTION_TELEMETRY_SMARTPORT | FUNCTION_TELEMETRY_LTM), FUNCTION_MSP);
    }
}
...
void mwArm(void)
{
    if ((armingFlags & (OK_TO_ARM))) {
        ...
        if (((1 << (BOXFAILSAFE)) & rcModeActivationMask)) {
            return;
        }
...
{
    if (AccInflightCalibrationArmed && (armingFlags & (ARMED)) && rcData[THROTTLE] > masterConfig.rxConfig.mincheck && !((1 << (BOXARM)) & rcModeActivationMask)) {
        InflightcalibratingA = 50;
        AccInflightCalibrationArmed = 0;
    }
    if (((1 << (BOXCALIB)) & rcModeActivationMask)) {
...
void processRx(void)
{
    ...
    if (feature(FEATURE_3D)) {
        if (!((1 << (BOXARM)) & rcModeActivationMask))
            mwDisarm();
    }
    ...
    if (throttleStatus == THROTTLE_LOW) {
        if (((1 << (BOXAIRMODE)) & rcModeActivationMask) && !failsafeIsActive() && (armingFlags & (ARMED))) {
    ...
    if ((((1 << (BOXANGLE)) & rcModeActivationMask) || (feature(FEATURE_FAILSAFE) && failsafeIsActive())) && (sensors(SENSOR_ACC))) {
    ...
    if (((1 << (BOXHORIZON)) & rcModeActivationMask) && canUseHorizonMode) {
    ...
    if ((flightModeFlags & (ANGLE_MODE)) || (flightModeFlags & (HORIZON_MODE))) {
        digitalLo(((GPIO_TypeDef *) ((((uint32_t)0x40000000) + 0x10000) + 0x0C00)), Pin_4);
    } else {
        digitalHi(((GPIO_TypeDef *) ((((uint32_t)0x40000000) + 0x10000) + 0x0C00)), Pin_4);
    }
    ...
    if (sensors(SENSOR_ACC) || sensors(SENSOR_MAG)) {
        ...
        if (((1 << (BOXHEADFREE)) & rcModeActivationMask)) {
            if (!(flightModeFlags & (HEADFREE_MODE))) {
        ...
        if (((1 << (BOXHEADADJ)) & rcModeActivationMask)) {
            headFreeModeHold = (attitude.values.yaw / 10);
        }
    }
    ...
    if (feature(FEATURE_TELEMETRY)) {
        if ((!masterConfig.telemetryConfig.telemetry_switch && (armingFlags & (ARMED))) ||
                (masterConfig.telemetryConfig.telemetry_switch && ((1 << (BOXTELEMETRY)) & rcModeActivationMask))) {
...
static _Bool haveProcessedAnnexCodeOnce = 0;
...
void taskMainPidLoopChecker(void) {
    ...
    if (masterConfig.gyroSync) {
        while (1) {
            if (gyroSyncCheckUpdate() || ((currentDeltaTime + (micros() - currentTime)) >= (targetLooptime + 100))) {
                break;
            }
        }
    }
