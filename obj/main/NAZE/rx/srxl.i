...
# 63 "./src/main/rx/srxl.c"
#define SRXL_START_OF_FRAME_BYTE_A1 (0xA1)
#define SRXL_START_OF_FRAME_BYTE_A2 (0xA2)
...
static _Bool srxlFrameReceived = 0;
static _Bool srxlDataIncoming = 0;
...
static volatile uint8_t srxlFrame[(16*2 + 3)];
static uint16_t srxlChannelData[16];
static uint16_t srxlReadRawRC(const rxRuntimeConfig_t *rxRuntimeConfig, uint8_t chan);
uint8_t srxlFrameStatus(void);

_Bool srxlInit(const rxConfig_t *rxConfig, rxRuntimeConfig_t *rxRuntimeConfig)
{
    (void)(rxConfig);
    ...
    for (i = 0; i < srxlFrameLength; i++) {
     ...
     if(crc_calc == 0) {
        ...
        for (i = 0; i < srxlChannelCount; i++) {
            ...
            srxlChannelData[i] = (800 + ((value * 1400) >> 12));
        }
