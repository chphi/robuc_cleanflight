...
static _Bool ibusFrameDone = 0;
static uint32_t ibusChannelData[10];
...
_Bool ibusInit(rxConfig_t *rxConfig, rxRuntimeConfig_t *rxRuntimeConfig, rcReadRawDataPtr *callback)
{
    ...
    rxRuntimeConfig->channelCount = 10;
    ...
    serialPort_t *ibusPort = openSerialPort(portConfig->identifier, FUNCTION_RX_SERIAL, ibusDataReceive, 115200, MODE_RX, SERIAL_NOT_INVERTED);

    return ibusPort != ((void *)0);
}
...
static void ibusDataReceive(uint16_t c)
{
    ...
    if (ibusFramePosition == 0 && c != 0x20)
        return;

    ibus[ibusFramePosition] = (uint8_t)c;

    if (ibusFramePosition == 32 - 1) {
        ibusFrameDone = 1;
    } else {
        ibusFramePosition++;
    }
...
uint8_t ibusFrameStatus(void)
{
    ...
    if (chksum == rxsum) {
        for (i = 0, offset = 2; i < 10; i++, offset += 2) {
