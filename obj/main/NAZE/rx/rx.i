...
_Bool sbusInit(const rxConfig_t *initialRxConfig, rxRuntimeConfig_t *rxRuntimeConfig);
...
_Bool sumdInit(const rxConfig_t *initialRxConfig, rxRuntimeConfig_t *rxRuntimeConfig);
...
_Bool sumhInit(const rxConfig_t *initialRxConfig, rxRuntimeConfig_t *rxRuntimeConfig);
...
_Bool xBusInit(const rxConfig_t *initialRxConfig, rxRuntimeConfig_t *rxRuntimeConfig);
...
_Bool ibusInit(const rxConfig_t *initialRxConfig, rxRuntimeConfig_t *rxRuntimeConfig);
# 61 "./src/main/rx/rx.c" 2
# 1 "./src/main/rx/srxl.h" 1
# 18 "./src/main/rx/srxl.h"
       



_Bool srxlInit(const rxConfig_t *initialRxConfig, rxRuntimeConfig_t *rxRuntimeConfig);
...
static _Bool rxDataReceived = 0;
static _Bool rxSignalReceived = 0;
static _Bool rxSignalReceivedNotDataDriven = 0;
static _Bool rxFlightChannelsValid = 0;
static _Bool rxIsInFailsafeMode = 1;
static _Bool rxIsInFailsafeModeNotDataDriven = 1;
...
extern const rxConfig_t pgResetTemplate_rxConfig; rxConfig_t rxConfig_System; extern const pgRegistry_t rxConfig_Registry; const pgRegistry_t rxConfig_Registry __attribute__ ((section(".pg_registry"), used, aligned(4))) = { .pgn = 24 | (0 << 12), .size = sizeof(rxConfig_t) | PGR_SIZE_SYSTEM_FLAG, .address = (uint8_t*)&rxConfig_System, .ptr = 0, .reset = {.ptr = (void*)&pgResetTemplate_rxConfig}, };

extern void pgResetFn_failsafeChannelConfigs(rxFailsafeChannelConfig_t *); rxFailsafeChannelConfig_t failsafeChannelConfigs_SystemArray[(18)]; extern const pgRegistry_t failsafeChannelConfigs_Registry; const pgRegistry_t failsafeChannelConfigs_Registry __attribute__ ((section(".pg_registry"), used, aligned(4))) = { .pgn = 43 | (0 << 12), .size = (sizeof(rxFailsafeChannelConfig_t) * (18)) | PGR_SIZE_SYSTEM_FLAG, .address = (uint8_t*)&failsafeChannelConfigs_SystemArray, .ptr = 0, .reset = {.fn = (pgResetFunc*)&pgResetFn_failsafeChannelConfigs}, };
extern void pgResetFn_channelRanges(rxChannelRangeConfiguration_t *); rxChannelRangeConfiguration_t channelRanges_SystemArray[4]; extern const pgRegistry_t channelRanges_Registry; const pgRegistry_t channelRanges_Registry __attribute__ ((section(".pg_registry"), used, aligned(4))) = { .pgn = 44 | (0 << 12), .size = (sizeof(rxChannelRangeConfiguration_t) * 4) | PGR_SIZE_SYSTEM_FLAG, .address = (uint8_t*)&channelRanges_SystemArray, .ptr = 0, .reset = {.fn = (pgResetFunc*)&pgResetFn_channelRanges}, };

const rxConfig_t pgResetTemplate_rxConfig __attribute__ ((section(".pg_resetdata"), used, aligned(2))) = { .sbus_inversion = 1, .midrc = 1500, .mincheck = 1100, .maxcheck = 1900, .rx_min_usec = 885, .rx_max_usec = 2115, .rssi_scale = 30, }
...
{
    for (int i = 0; i < (18); i++) {
        instance[i].mode = (i < 4) ? RX_FAILSAFE_MODE_AUTO : RX_FAILSAFE_MODE_HOLD;
        instance[i].step = (i == THROTTLE)
            ? ((constrain(rxConfig()->rx_min_usec, 750, 2250) - 750) / 25)
            : ((constrain(rxConfig()->midrc, 750, 2250) - 750) / 25);
    }
}
...
_Bool serialRxInit(const rxConfig_t *rxConfig, rxRuntimeConfig_t *rxRuntimeConfig);
...
static _Bool rxHaveValidFlightChannels(void)
{
    return (validFlightChannelMask == 0x0F);
}

static _Bool isPulseValid(uint16_t pulseDuration)
{
...
static void rxUpdateFlightChannelStatus(uint8_t channel, _Bool valid)
{
    if (channel < 4 && !valid) {
...
void rxInit(modeActivationCondition_t *modeActivationConditions)
{
    ...
    if (feature(FEATURE_RX_SERIAL)) {
        const _Bool enabled = serialRxInit(rxConfig(), &rxRuntimeConfig);
        if (!enabled) {
...
static _Bool isRxDataDriven(void)
{
...
static void resetRxSignalReceivedFlagIfNeeded(uint32_t currentTime)
{
    ...
    if (((int32_t)(currentTime - needRxSignalBefore) >= 0)) {
        rxSignalReceived = 0;
        rxSignalReceivedNotDataDriven = 0;
    }
}
...
void updateRx(uint32_t currentTime)
{
    ...
    if (feature(FEATURE_RX_SERIAL)) {
        ...
        if (frameStatus & RX_FRAME_COMPLETE) {
            ...
            needRxSignalBefore = currentTime + (1000000 / 10);
        }
    }
    ...
    if (feature(FEATURE_RX_MSP)) {
        ...
        if (frameStatus & RX_FRAME_COMPLETE) {
            rxDataReceived = 1;
            rxIsInFailsafeMode = 0;
            rxSignalReceived = 1;
            needRxSignalBefore = currentTime + (1000000 / 5);
        }
    }

    if (feature(FEATURE_RX_PPM)) {
        if (isPPMDataBeingReceived()) {
            rxSignalReceivedNotDataDriven = 1;
            rxIsInFailsafeModeNotDataDriven = 0;
            needRxSignalBefore = currentTime + (1000000 / 10);
...
{
    static uint16_t rcSamples[12][3];
    static _Bool rxSamplesCollected = 0;

    uint8_t currentSampleIndex = rcSampleIndex % 3;
...
static uint16_t getRxfailValue(uint8_t channel)
{
    ...
    if ( channel < 4 && (!feature(FEATURE_FAILSAFE)) ) {
    ...
    switch(mode) {
        ...
        case RX_FAILSAFE_MODE_SET:
            return (750 + 25 * failsafeChannelConfig->step);
    }
}
...
static uint16_t applyRxChannelRangeConfiguraton(int sample, rxChannelRangeConfiguration_t *range)
{
    ...
    sample = ((((750) > (sample) ? (750) : (sample))) < (2250) ? (((750) > (sample) ? (750) : (sample))) : (2250));
...
static void readRxChannelsApplyRanges(void)
{
    ...
    for (channel = 0; channel < rxRuntimeConfig.channelCount; channel++) {
        ...
        uint8_t rawChannel = calculateChannelRemapping(rxConfig()->rcmap, (sizeof(rxConfig()->rcmap) / sizeof((rxConfig()->rcmap)[0])), channel);
...
static void detectAndApplySignalLossBehaviour(void)
{
    int channel;
    uint16_t sample;
    _Bool useValueFromRx = 1;
    _Bool rxIsDataDriven = isRxDataDriven();
    ...
    for (channel = 0; channel < rxRuntimeConfig.channelCount; channel++) {
        ...
        sample = (useValueFromRx) ? rcRaw[channel] : 0;

        _Bool validPulse = isPulseValid(sample);
        ...
        if (!validPulse) {
            ...
            rcInvalidPulsPeriod[channel] = currentMilliTime + 300;
        }
    ...
    if ((rxFlightChannelsValid) && !(rcModeIsActive(BOXFAILSAFE) && feature(FEATURE_FAILSAFE))) {
        ...
        rxIsInFailsafeMode = rxIsInFailsafeModeNotDataDriven = 1;
...
void updateRSSIADC(uint32_t currentTime)
{
    ...
    static uint8_t adcRssiSamples[16];
    ...
    rssiUpdateAt = currentTime + (1000000 / 50);

    int16_t adcRssiMean = 0;
    uint16_t adcRssiSample = adcGetChannel(ADC_CHANNEL2);
    uint8_t rssiPercentage = adcRssiSample / rxConfig()->rssi_scale;

    adcRssiSampleIndex = (adcRssiSampleIndex + 1) % 16;
