...
_Bool sbusInit(rxConfig_t *initialRxConfig, rxRuntimeConfig_t *rxRuntimeConfig, rcReadRawDataPtr *callback);
...
_Bool sumdInit(rxConfig_t *rxConfig, rxRuntimeConfig_t *rxRuntimeConfig, rcReadRawDataPtr *callback);
...
_Bool sumhInit(rxConfig_t *rxConfig, rxRuntimeConfig_t *rxRuntimeConfig, rcReadRawDataPtr *callback);
...
_Bool xBusInit(rxConfig_t *rxConfig, rxRuntimeConfig_t *rxRuntimeConfig, rcReadRawDataPtr *callback);
...
_Bool ibusInit(rxConfig_t *rxConfig, rxRuntimeConfig_t *rxRuntimeConfig, rcReadRawDataPtr *callback);
...
void resetAllRxChannelRangeConfigurations(rxChannelRangeConfiguration_t *rxChannelRangeConfiguration) {
    ...
    for (int i = 0; i < 4; i++) {
        rxChannelRangeConfiguration->min = 1000;
        rxChannelRangeConfiguration->max = 2000;
...
static uint16_t getRxfailValue(uint8_t channel)
{
    ...
    switch(mode) {
        ...
        case RX_FAILSAFE_MODE_SET:
            return (750 + 25 * channelFailsafeConfiguration->step);
    }
}
...
static void readRxChannelsApplyRanges(void)
{
    ...
    for (channel = 0; channel < rxRuntimeConfig.channelCount; channel++) {
        ...
        uint8_t rawChannel = calculateChannelRemapping(rxConfig->rcmap, (sizeof(((rxConfig_t *)0)->rcmap) / sizeof(((rxConfig_t *)0)->rcmap[0])), channel);
...
static void detectAndApplySignalLossBehaviour(void)
{
    ...
    if ((rxFlightChannelsValid) && !(((1 << (BOXFAILSAFE)) & rcModeActivationMask) && feature(FEATURE_FAILSAFE))) {
