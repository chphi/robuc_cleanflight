...
static _Bool xBusFrameReceived = 0;
static _Bool xBusDataIncoming = 0;
...
static volatile uint8_t xBusFrame[33];
...
static void xBusUnpackModeBFrame(uint8_t offsetBytes)
{
    ...
    crc = ((uint16_t)xBusFrame[offsetBytes + 27 - 2]) << 8;
    crc = crc + ((uint16_t)xBusFrame[offsetBytes + 27 - 1]);
...
static void xBusDataReceive(uint16_t c)
{
    ...
    if ((xBusFramePosition == 0) && (c == (0xA1))) {
        xBusDataIncoming = 1;
    }


    if (xBusDataIncoming == 1) {
