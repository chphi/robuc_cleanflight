...
# 24 "./src/main/msp/msp_serial.c" 2
...
# 28 "./src/main/msp/msp_serial.c" 2
...
# 38 "./src/main/msp/msp_serial.c" 2
...
# 41 "./src/main/msp/msp_serial.c" 2
...
# 42 "./src/main/msp/msp_serial.c" 2
...
# 43 "./src/main/msp/msp_serial.c" 2

mspPostProcessFuncPtr mspPostProcessFn = ((void *)0);

mspPort_t mspPorts[2];
...
# 94 "./src/main/msp/msp_serial.c"
        serialPort_t *serialPort = openSerialPort(portConfig->identifier, function, ((void *)0),
...
{
    for(int i = 0; i < 2; i++) {
        resetMspPort(&mspPorts[i], ((void *)0));
    }
...
void mspSerialEncode(mspPort_t *msp, mspPacket_t *packet)
{
    ...
    csum = mspSerialChecksumBuf(csum, hdr + 3, 2);
    if(len > 0) {
...
static void mspSerialProcessReceivedCommand(mspPort_t *msp)
{
    ...
    mspPacket_t message = {
        .buf = {
            .ptr = outBuf,
            .end = (&(outBuf)[(sizeof(outBuf) / sizeof((outBuf)[0]))]),
        },
        .cmd = -1,
        .result = 0,
    };
...
# 211 "./src/main/msp/msp_serial.c"
static _Bool mspSerialProcessReceivedByte(mspPort_t *msp, uint8_t c)
{
    switch(msp->c_state) {
        default:
        case IDLE:
...
void mspSerialProcess(void)
{
    for (int i = 0; i < 2; i++) {
        ...
        while ((bytesWaiting = serialRxBytesWaiting(msp->port))) {
            ...
            _Bool consumed = mspSerialProcessReceivedByte(msp, c);
        ...
        if (msp->c_state == IDLE && msp->commandSenderFn && !bytesWaiting) {
            ...
            _Bool shouldSend = msp->commandSenderFn(command);

            if (shouldSend) {
                sbufSwitchToReader(&command->buf, outBufHead);

                mspSerialEncode(msp, command);
            }

            msp->commandSenderFn = ((void *)0);
        }
    }
}
