...
static void resetConf(void)
{
    ...
    featureSet(FEATURE_RX_PARALLEL_PWM | FEATURE_FAILSAFE | FEATURE_BLACKBOX);
...
# 148 "./src/main/fc/config.c"
    for (const pgRegistry_t *(reg) = __pg_registry_start; (reg) < __pg_registry_end; reg++) if(pgIsSystem(reg)) continue; else {
        for (int i = 1; i < 3; i++) {
            memcpy(reg->address + i * pgSize(reg), reg->address, pgSize(reg));
        }
    }
    for (int i = 1; i < 3; i++) {
        configureRateProfileSelection(i, i % 3);
    }
}
...
# 298 "./src/main/fc/config.c"
    if (!isSerialConfigValid(serialConfig())) {
        do { extern const pgRegistry_t serialConfig_Registry; pgResetCurrent(&serialConfig_Registry); } while(0);
    }
