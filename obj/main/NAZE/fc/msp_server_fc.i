...
# 1 "./src/main/common/pilot.h" 1
# 18 "./src/main/common/pilot.h"
#define CALLSIGN_LENGTH 14
typedef struct pilotConfig_s {
    uint8_t callsign[14 + 1];
} pilotConfig_t;

extern pilotConfig_t pilotConfig_System; static inline pilotConfig_t* pilotConfig(void) { return &pilotConfig_System; } struct _dummy;
...
# 21 "./src/main/drivers/pwm_mapping.h" 2
...
#define MSP_AMPERAGE_METERS 129
...
#define MSP_PASSTHROUGH_SERIAL 244
...
# 68 "./src/main/fc/msp_server_fc.c" 2
...
extern _Bool esc4wayExitRequested;
...
# 80 "./src/main/fc/msp_server_fc.c" 2
...
# 101 "./src/main/fc/msp_server_fc.c" 2
...
# 103 "./src/main/fc/msp_server_fc.c" 2
...
# 105 "./src/main/fc/msp_server_fc.c" 2
...
# 1 "./src/main/osd/osd_serial.h" 1
# 18 "./src/main/osd/osd_serial.h"
...
# 107 "./src/main/fc/msp_server_fc.c" 2
# 1 "./src/main/osd/osd_screen.h" 1
# 18 "./src/main/osd/osd_screen.h"
#define TEXT_SCREEN_CHAR uint8_t
...
# 108 "./src/main/fc/msp_server_fc.c" 2
# 1 "./src/main/osd/msp_server_osd.h" 1
# 18 "./src/main/osd/msp_server_osd.h"
...
# 113 "./src/main/fc/msp_server_fc.c" 2
...
# 115 "./src/main/fc/msp_server_fc.c" 2
...
{
    serialPassthrough(msp->port, passThroughPort, ((void *)0), ((void *)0));
}
...
#define BM(x) (1 << (x))
    const uint32_t rcModeCopyMask = (1 << (BOXHEADADJ)) | (1 << (BOXCAMSTAB)) | (1 << (BOXCAMTRIG)) | (1 << (BOXBEEPERON))
        ...
        | (1 << (BOXTELEMETRY)) | (1 << (BOXGTUNE)) | (1 << (BOXBLACKBOX)) | (1 << (BOXAIRMODE)) | (1 << (BOXVTX));
...
{
    sbuf_t *dst = &reply->buf;
# 479 "./src/main/fc/msp_server_fc.c"
    sbufWriteU8(dst, 0);
    sbufWriteU8(dst, 0);
    sbufWriteU8(dst, 0);
    sbufWriteU32(dst, 0);
...
static void serializeDataflashReadReply(mspPacket_t *reply, uint32_t address, int size)
{
    ...
    size = ((size) < (sbufBytesRemaining(dst)) ? (size) : (sbufBytesRemaining(dst)));
...
int mspServerCommandHandler(mspPacket_t *cmd, mspPacket_t *reply)
{
    ...
    switch (cmd->cmd) {
        ...
        case 104:
            for (unsigned i = 0; i < 8; i++) {
                sbufWriteU16(dst, i < 12 ? motor[i] : 0);
            }
            break;
        ...
        case 106:
            ...
            sbufWriteU32(dst, GPS_coord[0]);
            sbufWriteU32(dst, GPS_coord[1]);
        ...
        case 207: {
            ...
            gpsConfig()->provider = sbufReadU8(src);
            sbufReadU8(src);
            gpsConfig()->sbasMode = sbufReadU8(src);
        ...
        case 220:
            ...
            break;
# 1331 "./src/main/fc/msp_server_fc.c"
        case 88: {
# 1346 "./src/main/fc/msp_server_fc.c"
        ...
        case 81:
            ...
            break;
# 1397 "./src/main/fc/msp_server_fc.c"
        case 72:
        ...
        case 201:
            ...
            GPS_coord[0] = sbufReadU32(src);
            GPS_coord[1] = sbufReadU32(src);
    ...
    case 244:
        ...
        case 47:
            ...
            for (int i = 0; i < 16 && sbufBytesRemaining(src) >= 4; i++) {
                hsvColor_t *color = colors(i);
