...
#define ATOMIC_BARRIER(data) __extension__ void __UNIQL(__barrierEnd)(typeof(data) **__d) { __asm__ volatile ("\t# barrier(" #data ")  end\n" : : "m" (**__d)); } typeof(data) __attribute__((__cleanup__(__UNIQL(__barrierEnd)))) *__UNIQL(__barrier) = &data; __asm__ volatile ("\t# barrier (" #data ") start\n" : "+m" (*__UNIQL(__barrier)))
# 112 "./src/main/build/atomic.h"
#define ATOMIC_OR(ptr,val) __sync_fetch_and_or(ptr, val)
#define ATOMIC_AND(ptr,val) __sync_fetch_and_and(ptr, val)
# 28 "./src/main/fc/boot.c" 2
...
# 111 "./src/main/common/printf.h" 2
...
_Bool spiInit(SPI_TypeDef *instance);
...
# 1009 "./src/main/drivers/io_def_generated.h"
...
# 32 "./src/main/drivers/io_def.h" 2
# 76 "./src/main/drivers/io.h" 2
...
# 23 "./src/main/drivers/sonar_hcsr04.h" 2
...
_Bool usbCableIsInserted(void);
# 64 "./src/main/fc/boot.c" 2
...
_Bool isTransponderIrReady(void);

extern uint8_t transponderIrDMABuffer[((11 + 1) * 10 * 6)];
extern volatile uint8_t transponderIrDataTransferInProgress;
# 65 "./src/main/fc/boot.c" 2
...
# 128 "./src/main/fc/boot.c" 2


# 1 "./src/main/target/NAZE/hardware_revision.h" 1
# 18 "./src/main/target/NAZE/hardware_revision.h"
...
void SetSysClock(_Bool overclock);


extern const systemConfig_t pgResetTemplate_systemConfig; systemConfig_t systemConfig_System; extern const pgRegistry_t systemConfig_Registry; const pgRegistry_t systemConfig_Registry __attribute__ ((section(".pg_registry"), used, aligned(4))) = { .pgn = 18 | (0 << 12), .size = sizeof(systemConfig_t) | PGR_SIZE_SYSTEM_FLAG, .address = (uint8_t*)&systemConfig_System, .ptr = 0, .reset = {.ptr = (void*)&pgResetTemplate_systemConfig}, };
pwmRxConfig_t pwmRxConfig_System; extern const pgRegistry_t pwmRxConfig_Registry; const pgRegistry_t pwmRxConfig_Registry __attribute__ ((section(".pg_registry"), used, aligned(4))) = { .pgn = 100 | (0 << 12), .size = sizeof(pwmRxConfig_t) | PGR_SIZE_SYSTEM_FLAG, .address = (uint8_t*)&pwmRxConfig_System, .ptr = 0, .reset = {.ptr = 0}, };

const systemConfig_t pgResetTemplate_systemConfig __attribute__ ((section(".pg_resetdata"), used, aligned(2))) = { .i2c_highspeed = 1, }

 ;


flashchipConfig_t flashchipConfig_System; extern const pgRegistry_t flashchipConfig_Registry; const pgRegistry_t flashchipConfig_Registry __attribute__ ((section(".pg_registry"), used, aligned(4))) = { .pgn = 101 | (0 << 12), .size = sizeof(flashchipConfig_t) | PGR_SIZE_SYSTEM_FLAG, .address = (uint8_t*)&flashchipConfig_System, .ptr = 0, .reset = {.ptr = 0}, };
const flashchipConfig_t pgResetTemplate_flashchipConfig __attribute__ ((section(".pg_resetdata"), used, aligned(2))) = { .flashchip_id = 0, .flashchip_nsect = 0, .flashchip_pps = 0, }
...
void flashLedsAndBeep(void)
{
    ...
    digitalHi(led_config[0].gpio, led_config[0].pin);
    for (uint8_t i = 0; i < 10; i++) {
        digitalToggle(led_config[1].gpio, led_config[1].pin);
        digitalToggle(led_config[0].gpio, led_config[0].pin);
        delay(25);
        systemBeep(1);
        delay(25);
        systemBeep(0);
...
# 304 "./src/main/fc/boot.c"
    ...
    EXTIInit();
# 333 "./src/main/fc/boot.c"
    ledInit(0);



    beeperConfig_t beeperConfig = {
        .gpioPeripheral = ((uint32_t)0x00000004),
        .gpioPin = Pin_12,
        .gpioPort = ((GPIO_TypeDef *) ((((uint32_t)0x40000000) + 0x10000) + 0x0800)),
    ...
    if (hardwareRevision >= NAZE32_REV5) {
        ...
        beeperConfig.isInverted = 1;
    }


    beeperInit(&beeperConfig);
# 369 "./src/main/fc/boot.c"
...
# 388 "./src/main/fc/boot.c"
    ...
    const sonarHardware_t *sonarHardware = ((void *)0);
    sonarGPIOConfig_t sonarGPIOConfig;
    if (feature(FEATURE_SONAR)) {
        _Bool usingCurrentMeterIOPins = (feature(FEATURE_AMPERAGE_METER) && batteryConfig()->amperageMeterSource == AMPERAGE_METER_ADC);
    ...
    if (mixerConfig()->mixerMode == MIXER_AIRPLANE || mixerConfig()->mixerMode == MIXER_FLYING_WING || mixerConfig()->mixerMode == MIXER_CUSTOM_AIRPLANE)
        pwm_params.airplane = 1;
    else
        pwm_params.airplane = 0;
    ...
    spiInit(((SPI_TypeDef *) ((((uint32_t)0x40000000) + 0x10000) + 0x3000)));
    spiInit(((SPI_TypeDef *) (((uint32_t)0x40000000) + 0x3800)));
# 500 "./src/main/fc/boot.c"
    updateHardwareRevision();
# 509 "./src/main/fc/boot.c"
    if (hardwareRevision == NAZE32_SP) {
        serialRemovePort(SERIAL_PORT_SOFTSERIAL2);
    } else {
        serialRemovePort(SERIAL_PORT_UART3);
    }
# 530 "./src/main/fc/boot.c"
    if (hardwareRevision != NAZE32_SP) {
        i2cInit((I2CDEV_2));
    } else {
...
# 547 "./src/main/fc/boot.c"
    drv_adc_config_t adc_params;

    adc_params.channelMask = 0;


    adc_params.channelMask = (feature(FEATURE_VBAT) ? (1 << ADC_CHANNEL1) : 0);


    adc_params.channelMask |= (feature(FEATURE_RSSI_ADC) ? (1 << ADC_CHANNEL2) : 0);


    adc_params.channelMask |= (feature(FEATURE_AMPERAGE_METER) ? (1 << ADC_CHANNEL0) : 0);
# 573 "./src/main/fc/boot.c"
    adc_params.channelMask |= (hardwareRevision >= NAZE32_REV5) ? (1 << ADC_CHANNEL3) : 0;
...
# 588 "./src/main/fc/boot.c"
    ...
    const uint16_t pidPeriodUs = (1000000 / (gyro.sampleFrequencyHz));
...
# 759 "./src/main/fc/boot.c"
    latchActiveFeatures();
    motorControlEnable = 1;

    systemState |= SYSTEM_STATE_READY;
}
# 776 "./src/main/fc/boot.c"
...
void configureScheduler(void)
{
    ...
    uint16_t gyroPeriodUs = (1000000 / (gyro.sampleFrequencyHz));
    rescheduleTask(TASK_GYRO, gyroPeriodUs);
    setTaskEnabled(TASK_GYRO, 1);

    rescheduleTask(TASK_PID, gyroPeriodUs);
    setTaskEnabled(TASK_PID, 1);

    if (sensors(SENSOR_ACC)) {
        setTaskEnabled(TASK_ACCEL, 1);
        rescheduleTask(TASK_ACCEL, accSamplingInterval);
    }

    setTaskEnabled(TASK_ATTITUDE, sensors(SENSOR_ACC));
    setTaskEnabled(TASK_SERIAL, 1);
    setTaskEnabled(TASK_HARDWARE_WATCHDOG, 1);

    setTaskEnabled(TASK_BEEPER, 1);
