...
# 1 "./src/main/scheduler/scheduler.c"
# 18 "./src/main/scheduler/scheduler.c"
...
# 29 "./src/main/scheduler/scheduler.c" 2
...
# 31 "./src/main/scheduler/scheduler.c" 2
...
# 67 "./src/main/scheduler/scheduler.c"
static _Bool queueContains(cfTask_t *task)
{
...
static _Bool queueAdd(cfTask_t *task)
{
    if ((taskQueueSize >= taskCount) || queueContains(task)) {
        return 0;
    }
    for (unsigned int ii = 0; ii <= taskQueueSize; ++ii) {
        if (taskQueueArray[ii] == ((void *)0) || taskQueueArray[ii]->staticPriority < task->staticPriority) {
...
static _Bool queueRemove(cfTask_t *task)
{
    for (unsigned int ii = 0; ii < taskQueueSize; ++ii) {
        if (taskQueueArray[ii] == task) {
...
static inline cfTask_t *queueFirst(void)
{
    taskQueuePos = 0;
...
static inline cfTask_t *queueNext(void)
{
...
# 147 "./src/main/scheduler/scheduler.c"
}
...
void rescheduleTask(const int taskId, uint32_t newPeriodMicros)
{
    if (taskId == -1 || taskId < (int)taskCount) {
        cfTask_t *task = taskId == -1 ? currentTask : &cfTasks[taskId];
        task->desiredPeriod = ((100) > (newPeriodMicros) ? (100) : (newPeriodMicros));
    }
}

void setTaskEnabled(const int taskId, _Bool enabled)
{
    if (taskId == -1 || taskId < (int)taskCount) {
        cfTask_t *task = taskId == -1 ? currentTask : &cfTasks[taskId];
...
void scheduler(void)
{
    ...
    for (const cfTask_t *task = queueFirst(); task != ((void *)0) && task->staticPriority >= TASK_PRIORITY_REALTIME; task = queueNext()) {
    ...
    const _Bool outsideRealtimeGuardInterval = (timeToNextRealtimeTask > realtimeGuardInterval);


    cfTask_t *selectedTask = ((void *)0);
    uint16_t selectedTaskDynamicPriority = 0;


    uint16_t waitingTasks = 0;
    for (cfTask_t *task = queueFirst(); task != ((void *)0); task = queueNext()) {

        if (task->checkFunc != ((void *)0)) {
        ...
        if (task->dynamicPriority > selectedTaskDynamicPriority) {
            const _Bool taskCanBeChosenForScheduling =
    ...
    if (selectedTask != ((void *)0)) {
        ...
        selectedTask->maxExecutionTime = ((selectedTask->maxExecutionTime) > (taskExecutionTime) ? (selectedTask->maxExecutionTime) : (taskExecutionTime));
