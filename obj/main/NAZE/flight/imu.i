...
const imuConfig_t pgResetTemplate_imuConfig __attribute__ ((section(".pg_resetdata"), used, aligned(2))) = { .dcm_kp = 2500, .small_angle = 25, .max_angle_inclination = 500, }
...
void imuInit(void)
{
    ...
    gyroScale = gyro.scale * (3.14159265358979323846f / 180.0f);
...
static _Bool imuUseFastGains(void)
{
...
static void imuMahonyAHRSupdate(float dt, float gx, float gy, float gz,
                                ...
                                _Bool useMag, float mx, float my, float mz,
                                _Bool useYaw, float yawError)
{
...
static _Bool imuIsAccelerometerHealthy(void)
{
    int32_t axis;
...
static _Bool isMagnetometerHealthy(void)
{
...
static void imuCalculateEstimatedAttitude(void)
{
    ...
    else if ((stateFlags & (FIXED_WING)) && sensors(SENSOR_GPS) && (stateFlags & (GPS_FIX)) && GPS_numSat >= 5 && GPS_speed >= 300) {

        rawYawError = ((attitude.values.yaw - GPS_ground_course / 10.0f) * 0.0174532925f);
        useYaw = 1;
    }
