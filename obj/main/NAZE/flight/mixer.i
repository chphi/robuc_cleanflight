...
_Bool isMotorBrushed(uint16_t motorPwmRate);
...
# 20 "./src/main/drivers/pwm_mapping.h" 2
...
int16_t motor[12];
int16_t motor_disarmed[12];

_Bool motorLimitReached;

motorMixer_t currentMixer[12];

motorMixer_t customMotorMixer_SystemArray[12]; extern const pgRegistry_t customMotorMixer_Registry; const pgRegistry_t customMotorMixer_Registry __attribute__ ((section(".pg_registry"), used, aligned(4))) = { .pgn = 4 | (0 << 12), .size = (sizeof(motorMixer_t) * 12) | PGR_SIZE_SYSTEM_FLAG, .address = (uint8_t*)&customMotorMixer_SystemArray, .ptr = 0, .reset = {.ptr = 0}, };
extern const mixerConfig_t pgResetTemplate_mixerConfig; mixerConfig_t mixerConfig_System; extern const pgRegistry_t mixerConfig_Registry; const pgRegistry_t mixerConfig_Registry __attribute__ ((section(".pg_registry"), used, aligned(4))) = { .pgn = 20 | (0 << 12), .size = sizeof(mixerConfig_t) | PGR_SIZE_SYSTEM_FLAG, .address = (uint8_t*)&mixerConfig_System, .ptr = 0, .reset = {.ptr = (void*)&pgResetTemplate_mixerConfig}, };


extern const motor3DConfig_t pgResetTemplate_motor3DConfig; motor3DConfig_t motor3DConfig_System; extern const pgRegistry_t motor3DConfig_Registry; const pgRegistry_t motor3DConfig_Registry __attribute__ ((section(".pg_registry"), used, aligned(4))) = { .pgn = 26 | (0 << 12), .size = sizeof(motor3DConfig_t) | PGR_SIZE_SYSTEM_FLAG, .address = (uint8_t*)&motor3DConfig_System, .ptr = 0, .reset = {.ptr = (void*)&pgResetTemplate_motor3DConfig}, };

const motor3DConfig_t pgResetTemplate_motor3DConfig __attribute__ ((section(".pg_resetdata"), used, aligned(2))) = { .deadband3d_low = 1406, .deadband3d_high = 1514, .neutral3d = 1460, }
...
const mixerConfig_t pgResetTemplate_mixerConfig __attribute__ ((section(".pg_resetdata"), used, aligned(2))) = { .mixerMode = MIXER_QUADX, .pid_at_min_throttle = 1, .yaw_motor_direction = 1, .yaw_jump_prevention_limit = 200, .tri_unarmed_servo = 1, .servo_lowpass_freq = 400.0f, }
...
void mixTable(void)
{
    ...
    _Bool isFailsafeActive = failsafeIsActive();

    if (motorCount >= 4 && mixerConfig()->yaw_jump_prevention_limit < 500) {

        axisPID[FD_YAW] = constrain(axisPID[FD_YAW], -mixerConfig()->yaw_jump_prevention_limit - ((rcCommand[YAW]) > 0 ? (rcCommand[YAW]) : -(rcCommand[YAW])), mixerConfig()->yaw_jump_prevention_limit + ((rcCommand[YAW]) > 0 ? (rcCommand[YAW]) : -(rcCommand[YAW])));
    }

    if (rcModeIsActive(BOXAIRMODE)) {

        int16_t rollPitchYawMix[12];
        ...
        if (feature(FEATURE_3D)) {
            if (!(armingFlags & (ARMED))) throttlePrevious = rxConfig()->midrc;

            if ((rcData[THROTTLE] <= (rxConfig()->midrc - rcControlsConfig()->deadband3d_throttle))) {
                throttleMax = motor3DConfig()->deadband3d_low;
                throttleMin = motorConfig()->minthrottle;
                throttlePrevious = throttle = rcData[THROTTLE];
            } else if (rcData[THROTTLE] >= (rxConfig()->midrc + rcControlsConfig()->deadband3d_throttle)) {
                throttleMax = motorConfig()->maxthrottle;
                throttleMin = motor3DConfig()->deadband3d_high;
                throttlePrevious = throttle = rcData[THROTTLE];
            } else if ((throttlePrevious <= (rxConfig()->midrc - rcControlsConfig()->deadband3d_throttle))) {
