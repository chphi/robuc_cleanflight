...
static motorMixer_t currentMixer[12];
...
static biquad_t servoFilterState[8];
...
static void servoMixer(void)
{
    ...
    for (i = 0; i < servoRuleCount; i++) {
        ...
        if (currentServoMixer[i].box == 0 || ((1 << (BOXSERVO1 + currentServoMixer[i].box - 1)) & rcModeActivationMask)) {
...
void mixTable(void)
{
    ...
    if (motorCount >= 4 && mixerConfig->yaw_jump_prevention_limit < 500) {
        ...
        axisPID[YAW] = constrain(axisPID[YAW], -mixerConfig->yaw_jump_prevention_limit - ((rcCommand[YAW]) > 0 ? (rcCommand[YAW]) : -(rcCommand[YAW])), mixerConfig->yaw_jump_prevention_limit + ((rcCommand[YAW]) > 0 ? (rcCommand[YAW]) : -(rcCommand[YAW])));
    }

    if (((1 << (BOXAIRMODE)) & rcModeActivationMask)) {
...
# 954 "./src/main/flight/mixer.c"
    ...
    if (feature(FEATURE_SERVO_TILT)) {
        ...
        if (((1 << (BOXCAMSTAB)) & rcModeActivationMask)) {
