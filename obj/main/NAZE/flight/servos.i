...
# 66 "./src/main/flight/servos.c" 2

extern uint8_t motorCount;
extern motorMixer_t *customMixers;
extern mixer_t mixers[];
extern motorMixer_t currentMixer[12];
...
static biquadFilter_t servoFilterState[8];

servoMixer_t customServoMixer_SystemArray[(2 * 8)]; extern const pgRegistry_t customServoMixer_Registry; const pgRegistry_t customServoMixer_Registry __attribute__ ((section(".pg_registry"), used, aligned(4))) = { .pgn = 21 | (0 << 12), .size = (sizeof(servoMixer_t) * (2 * 8)) | PGR_SIZE_SYSTEM_FLAG, .address = (uint8_t*)&customServoMixer_SystemArray, .ptr = 0, .reset = {.ptr = 0}, };

extern void pgResetFn_servoProfile(servoProfile_t *); static servoProfile_t servoProfile_Storage[3]; servoProfile_t *servoProfile_ProfileCurrent; extern const pgRegistry_t servoProfile_Registry; const pgRegistry_t servoProfile_Registry __attribute__ ((section(".pg_registry"), used, aligned(4))) = { .pgn = 42 | (1 << 12), .size = sizeof(servoProfile_t) | PGR_SIZE_PROFILE_FLAG, .address = (uint8_t*)&servoProfile_Storage, .ptr = (uint8_t **)&servoProfile_ProfileCurrent, .reset = {.fn = (pgResetFunc*)&pgResetFn_servoProfile}, };

void pgResetFn_servoProfile(servoProfile_t *instance)
{
    for (int i = 0; i < 8; i++) {
        static const servoParam_t _reset_template_93 = { .min = 1000, .max = 2000, .middle = 1500, .rate = 100, .forwardFromChannel = (uint8_t)0xFF, }; memcpy((&instance->servoConf[i]), &_reset_template_93, sizeof(*(&instance->servoConf[i])));
...
const mixerRules_t servoMixers[] = {
    ...
    { 0, ((void *)0) },
    { 0, ((void *)0) },
    { (sizeof(servoMixerBI) / sizeof(servoMixer_t)), servoMixerBI },
    ...
    { 0, ((void *)0) },
    { 0, ((void *)0) },
    { (sizeof(servoMixerDual) / sizeof(servoMixer_t)), servoMixerDual },
...
static void forwardAuxChannelsToServos(uint8_t firstServoIndex)
{
    ...
    uint8_t servoOffset;
    for (servoOffset = 0; servoOffset < ((18) - 4) && channelOffset < (18); servoOffset++) {
...
void writeServos(void)
{
    ...
    switch (mixerConfig()->mixerMode) {
        ...
        case MIXER_AIRPLANE:
            for (int i = SERVO_ELEVATOR; i <= SERVO_FLAPS; i++) {
...
void filterServos(void)
{
    ...
    if (mixerConfig()->servo_lowpass_enable) {
        for (servoIdx = 0; servoIdx < 8; servoIdx++) {
