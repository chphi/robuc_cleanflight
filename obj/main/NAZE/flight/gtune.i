...
# 56 "./src/main/flight/gtune.c" 2

extern uint16_t pidDeltaUs;
extern uint8_t motorCount;
# 102 "./src/main/flight/gtune.c"
extern const gtuneConfig_t pgResetTemplate_gtuneConfig; static gtuneConfig_t gtuneConfig_Storage[3]; gtuneConfig_t *gtuneConfig_ProfileCurrent; extern const pgRegistry_t gtuneConfig_Registry; const pgRegistry_t gtuneConfig_Registry __attribute__ ((section(".pg_registry"), used, aligned(4))) = { .pgn = 15 | (0 << 12), .size = sizeof(gtuneConfig_t) | PGR_SIZE_PROFILE_FLAG, .address = (uint8_t*)&gtuneConfig_Storage, .ptr = (uint8_t **)&gtuneConfig_ProfileCurrent, .reset = {.ptr = (void*)&pgResetTemplate_gtuneConfig}, };

const gtuneConfig_t pgResetTemplate_gtuneConfig __attribute__ ((section(".pg_resetdata"), used, aligned(2))) = { .gtune_lolimP[FD_ROLL] = 10, .gtune_lolimP[FD_PITCH] = 10, .gtune_lolimP[FD_YAW] = 10, .gtune_hilimP[FD_ROLL] = 100, .gtune_hilimP[FD_PITCH] = 100, .gtune_hilimP[FD_YAW] = 100, .gtune_pwr = 0, .gtune_settle_time = 450, .gtune_average_cycles = 16, }
# 114 "./src/main/flight/gtune.c"
 ;
...
void init_Gtune(void)
{
 ...
 updateDelayCycles();
 for (i = 0; i < 3; i++) {
        if ((gtuneConfig()->gtune_hilimP[i] && gtuneConfig()->gtune_lolimP[i] > gtuneConfig()->gtune_hilimP[i]) || (motorCount < 4 && i == FD_YAW)) {
            gtuneConfig()->gtune_hilimP[i] = 0;
        }
...
void calculate_Gtune(uint8_t axis)
{
    ...
    if(rcCommand[axis] || (axis != FD_YAW && ((flightModeFlags & (ANGLE_MODE)) || (flightModeFlags & (HORIZON_MODE))))) {
        OldError[axis] = 0;
        ...
        if (time_skip[axis] == gtuneConfig()->gtune_average_cycles) {
            ...
            if (gtuneConfig()->gtune_hilimP[axis] && error && OldError[axis] && error != OldError[axis]) {
                diff_G = ((error) > 0 ? (error) : -(error)) - ((OldError[axis]) > 0 ? (OldError[axis]) : -(OldError[axis]));
                if ((error > 0 && OldError[axis] > 0) || (error < 0 && OldError[axis] < 0)) {
                    if (diff_G > threshP) {
                        if (axis == FD_YAW) {
                            result_P64[axis] += 256 + gtuneConfig()->gtune_pwr;
                        } else {
                    ...
                    if (((diff_G) > 0 ? (diff_G) : -(diff_G)) > threshP && axis != FD_YAW) {
                        result_P64[axis] -= 32;
                    }
                }
