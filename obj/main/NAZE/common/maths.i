...
float sinf(float x)
{
    ...
    float x2 = x * x;
    return x + x * x2 * (-1.666665710e-1f + x2 * (8.333017292e-3f + x2 * (-1.980661520e-4f + x2 * 2.600054768e-6f)));
}

float cosf(float x)
{
    return sinf(x + (0.5f * 3.14159265358979323846f));
}
...
int32_t applyDeadband(int32_t value, int32_t deadband)
{
    if (((value) > 0 ? (value) : -(value)) < deadband) {
        value = 0;
    } else if (value > 0) {
        value -= deadband;
    } else if (value < 0) {
