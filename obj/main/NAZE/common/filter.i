...
void pt1FilterInit(pt1Filter_t *filter, uint8_t f_cut, float dT)
{
    filter->RC = 1.0f / ( 2.0f * 3.14159265358979323846f * f_cut );
    filter->dT = dT;
}
...
static void biquadFilterInit(biquadFilter_t *filter, float filterFreq, uint32_t sampleDeltaUs, float Q, biquadFilterType_e filterType)
{
    ...
    const float omega = 2 * 3.14159265358979323846f * filterFreq / sampleHz;
...
{
    biquadFilterInit(filter, filterFreq, sampleDeltaUs, (1.0f / sqrtf(2.0f)), FILTER_LPF);
}
