...
float filterApplyPt1(float input, filterStatePt1_t *filter, uint8_t f_cut, float dT) {
 ...
 if (!filter->RC) {
  filter->RC = 1.0f / ( 2.0f * (float)3.14159265358979323846 * f_cut );
 }
...
void BiQuadNewLpf(float filterCutFreq, biquad_t *newState, uint32_t refreshRate)
{
    ...
    omega = 2 * 3.14159265358979323846f * filterCutFreq / sampleRate;
    sn = sinf(omega);
    cs = cosf(omega);
    alpha = sn * sinf(0.69314718055994530942f /2 * 1.9f * omega /sn);
