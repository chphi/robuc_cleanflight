...
# 1 "./src/main/config/config_eeprom.c"
# 17 "./src/main/config/config_eeprom.c"
...
# 18 "./src/main/config/config_eeprom.c" 2
...
# 29 "./src/main/config/config_eeprom.c" 2
...
void initEEPROM(void)
{
    ...
    ((void)sizeof(char[1 - 2*!!(__builtin_offsetof (packingTest_t, byte) != 0)]));
    ((void)sizeof(char[1 - 2*!!(__builtin_offsetof (packingTest_t, word) != 1)]));
    ((void)sizeof(char[1 - 2*!!(sizeof(packingTest_t) != 5)]));

    ((void)sizeof(char[1 - 2*!!(sizeof(configHeader_t) != 1)]));
    ...
    for (const pgRegistry_t *(reg) = __pg_registry_start; (reg) < __pg_registry_end; reg++) {
        ...
        for (configRecordFlags_e cls = cls_start; cls <= cls_end; cls++) {
            ...
            if (rec) {
                ...
                pgLoad(reg, profileIndex, rec->pg, rec->size - __builtin_offsetof (configRecord_t, pg), rec->version);
            } else {
...
static _Bool writeSettingsToEEPROM(void)
{
    ...
    for (const pgRegistry_t *(reg) = __pg_registry_start; (reg) < __pg_registry_end; reg++) {
        ...
        if (pgIsSystem(reg)) {
            ...
            for (uint8_t profileIndex = 0; profileIndex < 3; profileIndex++) {
                record.flags = 0;

                record.flags |= ((profileIndex + 1) & (0x3));
                config_streamer_write(&streamer, (uint8_t *)&record, sizeof(record));
                chk = updateChecksum(chk, (uint8_t *)&record, sizeof(record));
    ...
    _Bool success = config_streamer_finish(&streamer) == 0;
