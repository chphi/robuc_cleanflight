...
# 97 "./src/main/config/config.c"
#define FLASH_PAGE_SIZE ((uint16_t)0x400)
# 116 "./src/main/config/config.c"
#define FLASH_PAGE_COUNT ((FLASH_SIZE * 0x400) / FLASH_PAGE_SIZE)
# 130 "./src/main/config/config.c"
...
void resetPidProfile(pidProfile_t *pidProfile)
{
    ...
    pidProfile->yaw_p_limit = 500;
    pidProfile->dterm_cut_hz = 0;
    pidProfile->deltaMethod = 1;

    pidProfile->P_f[ROLL] = 1.4f;
...
{
    batteryConfig->vbatscale = 110;
    batteryConfig->vbatresdivval = 10;
    batteryConfig->vbatresdivmultiplier = 1;
...
# 369 "./src/main/config/config.c"
    ...
    masterConfig.soft_gyro_lpf_hz = 60;

    masterConfig.max_angle_inclination = 500;
    ...
    for (i = 0; i < (18); i++) {
        ...
        channelFailsafeConfiguration->mode = (i < 4) ? RX_FAILSAFE_MODE_AUTO : RX_FAILSAFE_MODE_HOLD;
        channelFailsafeConfiguration->step = (i == THROTTLE) ? ((constrain(masterConfig.rxConfig.rx_min_usec, 750, 2250) - 750) / 25) : ((constrain(masterConfig.rxConfig.midrc, 750, 2250) - 750) / 25);
    }
    ...
    masterConfig.failsafeConfig.failsafe_off_delay = 200;
    ...
    for (i = 0; i < 8; i++) {
        currentProfile->servoConf[i].min = 1000;
        currentProfile->servoConf[i].max = 2000;
        currentProfile->servoConf[i].middle = 1500;
        currentProfile->servoConf[i].rate = 100;
        currentProfile->servoConf[i].angleAtMin = 45;
        currentProfile->servoConf[i].angleAtMax = 45;
        currentProfile->servoConf[i].forwardFromChannel = (uint8_t)0xFF;
    }
    ...
    applyDefaultColors(masterConfig.colors, 16);
    applyDefaultLedStripConfig(masterConfig.ledConfigs);
# 511 "./src/main/config/config.c"
    masterConfig.blackbox_device = BLACKBOX_DEVICE_SERIAL;


    masterConfig.blackbox_rate_num = 1;
    masterConfig.blackbox_rate_denom = 1;
# 616 "./src/main/config/config.c"
    for (i = 1; i < 3; i++) {
    ...
    for (i = 1; i < 3; i++) {
        masterConfig.profile[i].defaultRateProfileIndex = i % 3;
    }
}
...
static _Bool isEEPROMContentValid(void)
{
    const master_t *temp = (const master_t *) (0x08000000 + (uint32_t)((((uint16_t)0x400) * ((128 * 0x400) / ((uint16_t)0x400))) - 0x1000));
    uint8_t checksum = 0;
...
void validateAndFixConfig(void)
{
    ...
    if (featureConfigured(FEATURE_SOFTSERIAL) && (
            ...
            || (((TIM_TypeDef *) (((uint32_t)0x40000000) + 0x0400)) == ((TIM_TypeDef *) (((uint32_t)0x40000000) + 0x0400)))
...
void readEEPROM(void)
{
    ...
    memcpy(&masterConfig, (char *) (0x08000000 + (uint32_t)((((uint16_t)0x400) * ((128 * 0x400) / ((uint16_t)0x400))) - 0x1000)), sizeof(master_t));

    if (masterConfig.current_profile_index > 3 - 1)
        masterConfig.current_profile_index = 0;

    setProfile(masterConfig.current_profile_index);

    if (currentProfile->defaultRateProfileIndex > 3 - 1)
...
void writeEEPROM(void)
{
    ...
    ((void)sizeof(char[1 - 2*!!(sizeof(master_t) > 0x1000)]));
    ...
    while (attemptsRemaining--) {
        ...
        for (wordOffset = 0; wordOffset < sizeof(master_t); wordOffset += 4) {
            if (wordOffset % ((uint16_t)0x400) == 0) {
                status = FLASH_ErasePage((0x08000000 + (uint32_t)((((uint16_t)0x400) * ((128 * 0x400) / ((uint16_t)0x400))) - 0x1000)) + wordOffset);
                if (status != FLASH_COMPLETE) {
                    break;
                }
            }

            status = FLASH_ProgramWord((0x08000000 + (uint32_t)((((uint16_t)0x400) * ((128 * 0x400) / ((uint16_t)0x400))) - 0x1000)) + wordOffset,
