...
_Bool ug2864hsweg01InitI2C(void);
...
# 206 "./src/main/io/display.c"
void updateTicker(void)
{
    static uint8_t tickerIndex = 0;
    i2c_OLED_set_xy((128 / (5 + 1)) - 1, 0);
...
void drawRxChannel(uint8_t channelIndex, uint8_t width)
{
    ...
    percentage = (constrain(rcData[channelIndex], 1000, 2000) - 1000) * 100 / (2000 - 1000);
...
void showRxPage(void)
{
    ...
    for (uint8_t channelIndex = 0; channelIndex < rxRuntimeConfig.channelCount && channelIndex < 14; channelIndex += 2) {
...
void showWelcomePage(void)
{
    ...
    tfp_sprintf(lineBuffer, "v%s (%s)", "1" "." "13" "." "0", shortGitRevision);
...
void showGpsPage(uint32_t now) {
    ...
    _Bool forceUpdateNow = (int32_t)(now - gpsPageForceUpdateAt) >= 0L;
    ...
    i2c_OLED_set_xy(((0) > ((((128 / (5 + 1)) - (sizeof(GPS_svinfo_cno) / sizeof(GPS_svinfo_cno[0]))) / 2)) ? (0) : ((((128 / (5 + 1)) - (sizeof(GPS_svinfo_cno) / sizeof(GPS_svinfo_cno[0]))) / 2))), rowIndex++);

    uint32_t index;
    for (index = 0; index < (sizeof(GPS_svinfo_cno) / sizeof(GPS_svinfo_cno[0])) && index < (128 / (5 + 1)); index++) {
        uint8_t bargraphOffset = ((uint16_t) GPS_svinfo_cno[index] * 7) / (55 - 1);
        bargraphOffset = ((bargraphOffset) < (7 - 1) ? (bargraphOffset) : (7 - 1));
        i2c_OLED_send_char((128 + 32) + bargraphOffset);
    }


    char fixChar = (stateFlags & (GPS_FIX)) ? 'Y' : 'N';
    tfp_sprintf(lineBuffer, "Sats: %d Fix: %c", GPS_numSat, fixChar);
    padLineBuffer();
    i2c_OLED_set_line(rowIndex++);
    i2c_OLED_send_string(lineBuffer);

    tfp_sprintf(lineBuffer, "La/Lo: %d/%d", GPS_coord[0] / 10000000L, GPS_coord[1] / 10000000L);
    padLineBuffer();
    i2c_OLED_set_line(rowIndex++);
    i2c_OLED_send_string(lineBuffer);
    ...
    padHalfLineBuffer();
    i2c_OLED_set_xy(((128 / (5 + 1)) / 2), rowIndex++);
    i2c_OLED_send_string(lineBuffer);
    ...
    strncpy(lineBuffer, gpsPacketLog, 21);
    padHalfLineBuffer();
    i2c_OLED_set_line(rowIndex++);
    i2c_OLED_send_string(lineBuffer);
# 451 "./src/main/io/display.c"
}
...
# 543 "./src/main/io/display.c"
void updateDisplay(void)
{
    uint32_t now = micros();
    static uint8_t previousArmedState = 0;

    _Bool updateNow = (int32_t)(now - nextDisplayUpdateAt) >= 0L;
    if (!updateNow) {
        return;
    }

    nextDisplayUpdateAt = now + ((1000 * 1000) / 5);

    _Bool armedState = (armingFlags & (ARMED)) ? 1 : 0;
    _Bool armedStateChanged = armedState != previousArmedState;
    ...
    if (armedState) {
        ...
        if (pageState.pageChanging && (pageState.pageFlags & PAGE_STATE_FLAG_CYCLE_ENABLED)) {
            ...
            pageState.cycleIndex = pageState.cycleIndex % (sizeof(cyclePageIds) / sizeof(cyclePageIds[0]));
    ...
    if (pageState.pageChanging) {
        pageState.pageFlags &= ~PAGE_STATE_FLAG_FORCE_PAGE_CHANGE;
        pageState.nextPageAt = now + ((1000 * 1000) * 5);
        pageState.nextToggleAt = now + ((1000 * 1000) / 2);
    ...
    if ((int32_t)(now - pageState.nextToggleAt) >= 0L) {
        ...
        pageState.nextToggleAt += ((1000 * 1000) / 2);
    }
...
void displayResetPageCycling(void)
{
    pageState.cycleIndex = (sizeof(cyclePageIds) / sizeof(cyclePageIds[0])) - 1;
