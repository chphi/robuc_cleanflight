...
# 1 "./src/main/io/serial_4way_stk500v2.c"
# 20 "./src/main/io/serial_4way_stk500v2.c"
...
# 21 "./src/main/io/serial_4way_stk500v2.c" 2
...
# 24 "./src/main/io/serial_4way_stk500v2.c" 2
...
# 26 "./src/main/io/serial_4way_stk500v2.c" 2
...
# 28 "./src/main/io/serial_4way_stk500v2.c" 2
...
# 32 "./src/main/io/serial_4way_stk500v2.c" 2
...
# 35 "./src/main/io/serial_4way_stk500v2.c" 2
...
# 38 "./src/main/io/serial_4way_stk500v2.c" 2
...
# 39 "./src/main/io/serial_4way_stk500v2.c" 2
...
# 40 "./src/main/io/serial_4way_stk500v2.c" 2
...
# 41 "./src/main/io/serial_4way_stk500v2.c" 2
...
static int8_t ReadBit(void)
{
    ...
    while (isEscHi(escSelected)) { if (cmp32(micros(), timeout_timer) > 0) goto timeout; };
    while (isEscLo(escSelected)) { if (cmp32(micros(), timeout_timer) > 0) goto timeout; };
...
static uint8_t StkRcvPacket(uint8_t *pstring, int maxLen)
{
    ...
    ckSumIn=0;
    if ((byte = ReadByte()) < 0 || (byte != 0x1B)) goto Err;
    if ((byte = ReadByte()) < 0 || (byte != SeqNumber)) goto Err;
    len = ReadByte() << 8;
    len |= ReadByte();
    if(len < 1 || len >= 256 + 4)
        goto Err;
    if ((byte = ReadByte()) < 0 || (byte != 0x0E)) goto Err;
    if ((byte = ReadByte()) < 0 || (byte != StkCmd)) goto Err;
    if ((byte = ReadByte()) < 0 || (byte != 0x00)) goto Err;
...
static uint8_t _CMD_SPI_MULTI_EX(uint8_t * resByte, uint8_t subcmd, uint16_t addr)
{
    ...
    StkSendByte(subcmd);
    StkSendByte(addr >> 8);
    StkSendByte(addr & 0xff);
    StkSendByte(0);
    StkSendPacketFooter();
    if (StkRcvPacket(stkInBuf, sizeof(stkInBuf))) {
        if ((stkInBuf[0] == 0x00)
            && ((stkInBuf[1] == subcmd) || (stkInBuf[1] == 0x00 ))
            && (stkInBuf[2] == 0x00)) {
            *resByte = stkInBuf[3];
        }
        return 1;
...
uint8_t Stk_ConnectEx(escDeviceInfo_t *pDeviceInfo)
{
    ...
    for(unsigned i = 0; i < sizeof(signature); i++) {
        if (!_CMD_SPI_MULTI_EX(&signature[i], 0x30, i))
            return 0;
    }
