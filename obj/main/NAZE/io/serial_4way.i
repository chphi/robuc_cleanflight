...
# 23 "./src/main/io/serial_4way.c" 2
...
# 26 "./src/main/io/serial_4way.c" 2
...
# 34 "./src/main/io/serial_4way.c" 2
...
# 36 "./src/main/io/serial_4way.c" 2
...
# 39 "./src/main/io/serial_4way.c" 2
...
# 19 "./src/main/io/serial_4way_impl.h"
typedef struct {
    GPIO_TypeDef* gpio;
    uint16_t pinpos;
    uint16_t pin;
...
_Bool isEscHi(uint8_t selEsc);
_Bool isEscLo(uint8_t selEsc);
...
# 42 "./src/main/io/serial_4way.c" 2
...
# 43 "./src/main/io/serial_4way.c" 2
...
uint8_t escSelected;
escHardware_t escHardware[12];

_Bool esc4wayExitRequested = 0;

static escDeviceInfo_t deviceInfo;

static _Bool isMcuConnected(void)
{
    ...
    for (int i = 0; i < pwmIOConfiguration->ioCount; i++) {
        if ((pwmIOConfiguration->ioConfigurations[i].flags & PWM_PF_MOTOR) == PWM_PF_MOTOR) {
            if(motor[pwmIOConfiguration->ioConfigurations[i].index] > 0) {
                ...
                escHardware[escIdx].gpio_config_INPUT.speed = Speed_2MHz;
...
{
    for(int i = 0; i < escCount; i++) {
        escHardware[i].gpio_config_OUTPUT.mode = Mode_AF_PP;
        setEscOutput(i);
        setEscLo(i);
    }
    escCount = 0;
...
# 204 "./src/main/io/serial_4way.c"
typedef enum {

    cmd_Remote_Escape = 0x2E,
    cmd_Local_Escape = 0x2F,
} syn_4way_e;
# 243 "./src/main/io/serial_4way.c"
...
static _Bool signatureMatch(uint16_t signature, uint16_t *list)
{
    for(; *list; list++)
        if(signature == *list)
            return 1;
...
esc4wayAck_e esc4wayProcessCmd(esc4wayCmd_e command, uint16_t addr, uint8_t *data, int inLen, int *outLen)
{
    ...
    switch(command) {
        ...
        case cmd_InterfaceGetName:
            ...
            memcpy(data, "m4wFCIntf", strlen("m4wFCIntf"));
            *outLen = strlen("m4wFCIntf");
            return esc4wayAck_OK;

        case cmd_InterfaceGetVersion:

            data[0] = (uint8_t)(14 * 10 + 4);
            data[1] = (uint8_t)(4);
            *outLen = 2;
            return esc4wayAck_OK;

        case cmd_InterfaceExit:
            esc4wayExitRequested = 1;
            return esc4wayAck_OK;
...
#define INTFMEM(interface,memory) (((interface) << 1) | (memory))
        ...
        case cmd_DeviceRead: {
            ...
            if(len == 0)
                len = 0x100;
            ioMem.len = len;
            switch((((currentInterfaceMode) << 1) | ((command == cmd_DeviceRead) ? 0 : 1))) {
        ...
        case cmd_DeviceWriteEEprom:
            ioMem.len = inLen;
            switch ((((currentInterfaceMode) << 1) | ((command == cmd_DeviceWrite) ? 0 : 1))) {
