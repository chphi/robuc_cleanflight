...
# 25 "./src/main/io/serial_msp.c" 2
# 1 "./src/main/debug.h" 1
# 18 "./src/main/debug.h"
...
# 75 "./src/main/io/serial_msp.c" 2
# 1 "./src/main/flight/altitudehold.h" 1
# 24 "./src/main/flight/altitudehold.h"
...
# 89 "./src/main/io/serial_msp.c" 2


# 1 "./src/main/io/serial_msp.h" 1
# 18 "./src/main/io/serial_msp.h"
       
# 59 "./src/main/io/serial_msp.h"
...
static const char * const boardIdentifier = "AFNA";
...
# 92 "./src/main/io/serial_msp.c" 2


# 1 "./src/main/io/serial_1wire.h" 1
# 21 "./src/main/io/serial_1wire.h"
...
const char * const flightControllerIdentifier = "CLFL";
...
static const box_t boxes[CHECKBOX_ITEM_COUNT + 1] = {
    ...
    { CHECKBOX_ITEM_COUNT, ((void *)0), 0xFF }
};
...
static _Bool isRebootScheduled = 0;
...
void mspAllocateSerialPorts(serialConfig_t *serialConfig)
{
    ...
    while (portConfig && portIndex < 2) {
        ...
        serialPort = openSerialPort(portConfig->identifier, FUNCTION_MSP, ((void *)0), baudRates[portConfig->msp_baudrateIndex], MODE_RXTX, SERIAL_NOT_INVERTED);
        if (serialPort) {
...
static uint32_t packFlightModeFlags(void)
{
    ...
    junk = 0;
    tmp = ((flightModeFlags & (ANGLE_MODE)) == 0 ? 0 : 1) << BOXANGLE |
        ((flightModeFlags & (HORIZON_MODE)) == 0 ? 0 : 1) << BOXHORIZON |
        ((flightModeFlags & (BARO_MODE)) == 0 ? 0 : 1) << BOXBARO |
        ((flightModeFlags & (MAG_MODE)) == 0 ? 0 : 1) << BOXMAG |
        ((flightModeFlags & (HEADFREE_MODE)) == 0 ? 0 : 1) << BOXHEADFREE |
        (((1 << (BOXHEADADJ)) & rcModeActivationMask) == 0 ? 0 : 1) << BOXHEADADJ |
        (((1 << (BOXCAMSTAB)) & rcModeActivationMask) == 0 ? 0 : 1) << BOXCAMSTAB |
        (((1 << (BOXCAMTRIG)) & rcModeActivationMask) == 0 ? 0 : 1) << BOXCAMTRIG |
        ((flightModeFlags & (GPS_HOME_MODE)) == 0 ? 0 : 1) << BOXGPSHOME |
        ((flightModeFlags & (GPS_HOLD_MODE)) == 0 ? 0 : 1) << BOXGPSHOLD |
        ((flightModeFlags & (PASSTHRU_MODE)) == 0 ? 0 : 1) << BOXPASSTHRU |
        (((1 << (BOXBEEPERON)) & rcModeActivationMask) == 0 ? 0 : 1) << BOXBEEPERON |
        (((1 << (BOXLEDMAX)) & rcModeActivationMask) == 0 ? 0 : 1) << BOXLEDMAX |
        (((1 << (BOXLEDLOW)) & rcModeActivationMask) == 0 ? 0 : 1) << BOXLEDLOW |
        (((1 << (BOXLLIGHTS)) & rcModeActivationMask) == 0 ? 0 : 1) << BOXLLIGHTS |
        (((1 << (BOXCALIB)) & rcModeActivationMask) == 0 ? 0 : 1) << BOXCALIB |
        (((1 << (BOXGOV)) & rcModeActivationMask) == 0 ? 0 : 1) << BOXGOV |
        (((1 << (BOXOSD)) & rcModeActivationMask) == 0 ? 0 : 1) << BOXOSD |
        (((1 << (BOXTELEMETRY)) & rcModeActivationMask) == 0 ? 0 : 1) << BOXTELEMETRY |
        (((1 << (BOXGTUNE)) & rcModeActivationMask) == 0 ? 0 : 1) << BOXGTUNE |
        ((flightModeFlags & (SONAR_MODE)) == 0 ? 0 : 1) << BOXSONAR |
        ((armingFlags & (ARMED)) == 0 ? 0 : 1) << BOXARM |
        (((1 << (BOXBLACKBOX)) & rcModeActivationMask) == 0 ? 0 : 1) << BOXBLACKBOX |
        ((flightModeFlags & (FAILSAFE_MODE)) == 0 ? 0 : 1) << BOXFAILSAFE |
        (((1 << (BOXAIRMODE)) & rcModeActivationMask) == 0 ? 0 : 1) << BOXAIRMODE;
...
static _Bool processOutCommand(uint8_t cmdMSP)
{
    uint32_t i;
    ...
    case 103:
        s_struct((uint8_t *)&servo, 8 * 2);
        break;
    case 120:
        headSerialReply(8 * sizeof(servoParam_t));
        for (i = 0; i < 8; i++) {
        ...
        break;
    case 241:
        headSerialReply((2 * 8) * sizeof(servoMixer_t));
        for (i = 0; i < (2 * 8); i++) {
    ...
    case 111:
        ...
        for (i = 0 ; i < 3; i++) {
            serialize8(currentControlRateProfile->rates[i]);
        }
    ...
    case 114:
        ...
        serialize8(masterConfig.gpsConfig.provider);
        serialize8(0);
        serialize8(masterConfig.gpsConfig.sbasMode);
    ...
    case 106:
        headSerialReply(16);
        serialize8((stateFlags & (GPS_FIX)));
        serialize8(GPS_numSat);
        serialize32(GPS_coord[0]);
        serialize32(GPS_coord[1]);
    ...
    case 254:
        headSerialReply(4 * sizeof(debug[0]));
    ...
    case 160:
        headSerialReply(12);
        serialize32((*(uint32_t*)0x1FFFF7E8));
        serialize32((*(uint32_t*)0x1FFFF7EC));
        serialize32((*(uint32_t*)0x1FFFF7F0));
        break;
    ...
    case 77:
        ...
        for (i = 0; i < rxRuntimeConfig.channelCount; i++) {
            ...
            serialize16((750 + 25 * masterConfig.rxConfig.failsafe_channel_configurations[i].step));
        }
        break;
    ...
    case 48:
        headSerialReply(32 * 7);
        for (i = 0; i < 32; i++) {
            ledConfig_t *ledConfig = &masterConfig.ledConfigs[i];
            serialize16((ledConfig->flags & ( LED_DIRECTION_NORTH | LED_DIRECTION_EAST | LED_DIRECTION_SOUTH | LED_DIRECTION_WEST | LED_DIRECTION_UP | LED_DIRECTION_DOWN )) >> 0);
            serialize16((ledConfig->flags & ( LED_FUNCTION_INDICATOR | LED_FUNCTION_WARNING | LED_FUNCTION_FLIGHT_MODE | LED_FUNCTION_ARM_STATE | LED_FUNCTION_THROTTLE | LED_FUNCTION_THRUST_RING | LED_FUNCTION_COLOR )) >> 6);
            serialize8(((ledConfig->xy >> 4) & (0x0F)));
            serialize8(((ledConfig->xy >> 0) & (0x0F)));
...
static _Bool processInCommand(void)
{
    uint32_t i;
    uint16_t tmp;
    uint8_t rate;
    ...
    case 204:
        if (currentPort->dataSize >= 10) {
            ...
            for (i = 0; i < 3; i++) {
                rate = read8();
                currentControlRateProfile->rates[i] = ((rate) < (i == FD_YAW ? 255 : 100) ? (rate) : (i == FD_YAW ? 255 : 100));
            }
            rate = read8();
    ...
    case 207:
        ...
        masterConfig.gpsConfig.provider = read8();
        read8();
        masterConfig.gpsConfig.sbasMode = read8();
        ...
        masterConfig.batteryConfig.vbatmincellvoltage = read8();
...
# 1497 "./src/main/io/serial_msp.c"
    ...
    case 209:
        ...
        if (wp_no == 0) {
            GPS_home[0] = lat;
            GPS_home[1] = lon;
            disableFlightMode(GPS_HOME_MODE);
    ...
    case 78:
        ...
        if (i < (18)) {
            ...
            masterConfig.rxConfig.failsafe_channel_configurations[i].step = ((constrain(read16(), 750, 2250) - 750) / 25);
        } else {
            headSerialError(0);
        }
        break;
    ...
    case 49:
        {
            i = read8();
            if (i >= 32 || currentPort->dataSize != (1 + 7)) {
                headSerialError(0);
                break;
            }
            ledConfig_t *ledConfig = &masterConfig.ledConfigs[i];
            ...
            mask = read16();
            ledConfig->flags = (mask << 0) & ( LED_DIRECTION_NORTH | LED_DIRECTION_EAST | LED_DIRECTION_SOUTH | LED_DIRECTION_WEST | LED_DIRECTION_UP | LED_DIRECTION_DOWN );

            mask = read16();
            ledConfig->flags |= (mask << 6) & ( LED_FUNCTION_INDICATOR | LED_FUNCTION_WARNING | LED_FUNCTION_FLIGHT_MODE | LED_FUNCTION_ARM_STATE | LED_FUNCTION_THROTTLE | LED_FUNCTION_THRUST_RING | LED_FUNCTION_COLOR );

            mask = read8();
            ledConfig->xy = ((mask & (0x0F)) << 4);

            mask = read8();
            ledConfig->xy |= ((mask & (0x0F)) << 0);
...
static _Bool mspProcessReceivedData(uint8_t c)
{
...
void mspProcess(void)
{
    ...
    for (portIndex = 0; portIndex < 2; portIndex++) {
        ...
        while (serialRxBytesWaiting(mspSerialPort)) {
            ...
            _Bool consumed = mspProcessReceivedData(c);

            if (!consumed && !(armingFlags & (ARMED))) {
