...
static void shiftPacketLog(void)
{
    ...
    for (i = (sizeof(gpsPacketLog) / sizeof((gpsPacketLog)[0])) - 1; i > 0 ; i--) {
        gpsPacketLog[i] = gpsPacketLog[i-1];
    }
}

static void gpsNewData(uint16_t c);
static _Bool gpsNewFrameNMEA(char c);
static _Bool gpsNewFrameUBLOX(uint8_t data);
...
void gpsInit(serialConfig_t *initialSerialConfig, gpsConfig_t *initialGpsConfig)
{
    ...
    while (gpsInitData[gpsData.baudrateIndex].baudrateIndex != gpsPortConfig->gps_baudrateIndex) {
        ...
        if (gpsData.baudrateIndex >= (sizeof(gpsInitData) / sizeof(gpsInitData[0]))) {
            gpsData.baudrateIndex = 0;
            break;
        }
    }
    ...
    gpsPort = openSerialPort(gpsPortConfig->identifier, FUNCTION_GPS, ((void *)0), gpsInitData[gpsData.baudrateIndex].baudrateIndex, mode, SERIAL_NOT_INVERTED);
    if (!gpsPort) {
        featureClear(FEATURE_GPS);
        return;
    }
...
void gpsInitUblox(void)
{
    ...
    switch (gpsData.state) {
        case GPS_INITIALIZING:
            now = millis();
            if (now - gpsData.state_ts < (200))
                return;

            if (gpsData.state_position < (GPS_BAUDRATE_9600 + 1)) {
        ...
        case GPS_CONFIGURE:
            ...
            if (gpsData.messageState >= (GPS_MESSAGE_STATE_MAX + 1)) {
    ...
    switch (gpsData.state) {
        ...
        case GPS_LOST_COMMUNICATION:
            ...
            if (gpsConfig->autoBaud) {
                ...
                gpsData.baudrateIndex++;
                gpsData.baudrateIndex %= (GPS_BAUDRATE_9600 + 1);
            }
            gpsData.lastMessage = millis();
        ...
        case GPS_RECEIVING_DATA:
            ...
            if (millis() - gpsData.lastMessage > (2500)) {
...
# 455 "./src/main/io/gps.c"
#define NO_FRAME 0
#define FRAME_GGA 1
#define FRAME_RMC 2
#define FRAME_GSV 3
# 502 "./src/main/io/gps.c"
...
static _Bool gpsNewFrameNMEA(char c)
{
    ...
    static uint8_t checksum_param, gps_frame = 0;
                    ...
                    case 1:
                      *gpsPacketLogChar = 'g';
                      ...
                      if ((stateFlags & (GPS_FIX))) {
                            GPS_coord[0] = gps_Msg.latitude;
                            GPS_coord[1] = gps_Msg.longitude;
...
static _Bool _skip_packet;
static uint8_t _step;
static uint8_t _msg_id;
static uint16_t _payload_length;
static uint16_t _payload_counter;

static _Bool next_fix;
static uint8_t _class;


static _Bool _new_position;


static _Bool _new_speed;
# 847 "./src/main/io/gps.c"
...
static _Bool UBLOX_parse_gps(void)
{
    uint32_t i;
    ...
    *gpsPacketLogChar = '!';
    ...
    case MSG_POSLLH:
        *gpsPacketLogChar = 'P';
        ...
        GPS_coord[1] = _buffer.posllh.longitude;
        GPS_coord[0] = _buffer.posllh.latitude;
