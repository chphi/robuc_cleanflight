...
# 25 "./src/main/io/serial_4way_avrootloader.c" 2
...
# 28 "./src/main/io/serial_4way_avrootloader.c" 2
...
# 29 "./src/main/io/serial_4way_avrootloader.c" 2
...
# 37 "./src/main/io/serial_4way_avrootloader.c" 2
...
# 39 "./src/main/io/serial_4way_avrootloader.c" 2
...
static int suart_getc(void)
{
    ...
    uint32_t wait_time = micros() + 2000;
    while (isEscHi(escSelected)) {
...
static void suart_putc(uint8_t byte)
{
    ...
    while(1) {
        if(bitmask & 1)
            setEscHi(escSelected);
        else
            setEscLo(escSelected);
        btime = btime + 52;
        bitmask >>= 1;
        if (bitmask == 0)
            break;
...
static uint8_t BL_ReadBuf(uint8_t *pstring, int len, _Bool checkCrc)
{
    int crc = 0;
    int c;
...
static void BL_SendBuf(uint8_t *pstring, int len, _Bool appendCrc)
{
    setEscOutput(escSelected);
    uint16_t crc = 0;
    for(int i = 0; i < len; i++) {
...
#define DevSignLo (BOOT_MSG_LEN + 1)
    ...
    uint8_t bootInfo[4 + 4];
    static const uint8_t bootMsgCheck[4 - 1] = "471";
    ...
    BL_SendBuf(bootInit, sizeof(bootInit), 0);
    if (!BL_ReadBuf(bootInfo, sizeof(bootInfo), 0))
        return 0;
    ...
    pDeviceInfo->signature2 = bootInfo[4 - 1];
    pDeviceInfo->signature = (bootInfo[(4)] << 8) | bootInfo[(4 + 1)];
    return 1;
}
...
static uint8_t BL_SendCMDSetAddress(ioMem_t *pMem)
{
    ...
    if((pMem->addr == 0xffff))
        return 1;
    uint8_t sCMD[] = {0xFF, 0, pMem->addr >> 8, pMem->addr & 0xff };
    BL_SendBuf(sCMD, sizeof(sCMD), 1);
...
static uint8_t BL_SendCMDSetBuffer(ioMem_t *pMem)
{
    ...
    uint8_t sCMD[] = {0xFE, 0, len >> 8, len & 0xff};
    BL_SendBuf(sCMD, sizeof(sCMD), 1);
    if (BL_GetACK(2) != 0xFF)
        return 0;
    BL_SendBuf(pMem->data, len, 1);
...
static uint8_t BL_ReadA(uint8_t cmd, ioMem_t *pMem)
{
    ...
    BL_SendBuf(sCMD, sizeof(sCMD), 1);
    return BL_ReadBuf(pMem->data, len, 1);
}
...
uint8_t BL_PageErase(ioMem_t *pMem)
{
    ...
    BL_SendBuf(sCMD, sizeof(sCMD), 1);
    return BL_GetACK(40 * 1000 / 2000) == 0x30;
}

uint8_t BL_WriteEEprom(ioMem_t *pMem)
{
    return BL_WriteA(0x05, pMem, 3000 * 1000 / 2000);
}

uint8_t BL_WriteFlash(ioMem_t *pMem)
{
    return BL_WriteA(0x01, pMem, 40 * 1000 / 2000);
}
